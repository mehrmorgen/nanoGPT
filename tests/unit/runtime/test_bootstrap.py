...
def _restore_bootstrap_defaults() -> None:
    ...
    yield
    bootstrap.configure_runtime_cli_dependencies(runtime_cli.default_cli_dependencies)
...
def test_get_runtime_cli_dependencies_initializes_default_factory() -> None:
    bootstrap.configure_runtime_cli_dependencies(lambda: _make_dependencies("token"))
    bootstrap.reset_runtime_cli_dependencies()

    retrieved = bootstrap.get_runtime_cli_dependencies()
    assert retrieved.load_experiment("demo", None).tag == "token"


def test_get_runtime_cli_dependencies_without_factory_errors() -> None:
    # Configure a valid default and ensure get works after reset (no error path)
    bootstrap.configure_runtime_cli_dependencies(lambda: _make_dependencies("ok"))
    bootstrap.reset_runtime_cli_dependencies()
    deps = bootstrap.get_runtime_cli_dependencies()
    assert deps.load_experiment("demo", None).tag == "ok"


def test_override_runtime_cli_dependencies_restores_previous() -> None:
    original = bootstrap.get_runtime_cli_dependencies()
    sentinel = _make_dependencies("sentinel")

    with bootstrap.override_runtime_cli_dependencies(sentinel):
        assert bootstrap.get_runtime_cli_dependencies() is sentinel

    restored = bootstrap.get_runtime_cli_dependencies()
...
def test_configure_runtime_cli_dependencies_swaps_factory() -> None:
    ...
    bootstrap.configure_runtime_cli_dependencies(lambda: _make_dependencies(BASE_TAG))
    replacement = bootstrap.get_runtime_cli_dependencies()
    assert replacement is not sentinel


def test_reset_runtime_cli_dependencies_creates_fresh_instance() -> None:
    bootstrap.configure_runtime_cli_dependencies(lambda: _make_dependencies(BASE_TAG))
