...
def _dummy_deps(marker: str = "deps") -> CLIDependencies:
    return CLIDependencies(
        ...
        run_prepare=lambda experiment, cfg, path, shared, engine: marker,
        run_train=lambda experiment, cfg, path, shared, engine: marker,
        run_sample=lambda experiment, cfg, path, shared, engine: marker,
    )


def test_reset_runtime_cli_dependencies_without_default_resets_to_none() -> None:
    """Reset with no default factory should leave dependencies unset."""
    fresh = importlib.reload(bootstrap)
    fresh.reset_runtime_cli_dependencies()
    with pytest.raises(RuntimeError):
        fresh.get_runtime_cli_dependencies()


def test_get_runtime_cli_dependencies_raises_when_unconfigured() -> None:
...
def test_reset_runtime_cli_dependencies_clears_cached_instance() -> None:
    ...
    fresh.configure_runtime_cli_dependencies(factory_one)
    deps_first = fresh.get_runtime_cli_dependencies()
    ...
    fresh.configure_runtime_cli_dependencies(factory_two)
    deps_second = fresh.get_runtime_cli_dependencies()

    # factory_one called once during configure and once when fetching deps_first
    # factory_two called once when fetching deps_second
    assert calls == ["one", "one", "two"]
    assert deps_first is not deps_second


def test_get_runtime_cli_dependencies_uses_default_factory_once() -> None:
    ...
    fresh.configure_runtime_cli_dependencies(factory)

    deps = fresh.get_runtime_cli_dependencies()
    assert isinstance(deps, CLIDependencies)
    assert calls == ["called"]

    # cached path should not call factory again
    deps_again = fresh.get_runtime_cli_dependencies()
