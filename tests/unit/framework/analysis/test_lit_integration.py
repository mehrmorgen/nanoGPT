...
"""Unit tests for `ml_playground.framework.analysis.lit.integration`."""
...
def test_load_lit_components_uses_fake_modules() -> None:
    ...
    assert getattr(dataset_mod, "Dataset", None) is not None
    assert model_mod.__name__ == "lit_nlp.api.model"
    assert getattr(model_mod, "Model", None) is not None
...
def test_import_lit_server_returns_fake_module() -> None:
    ...
    with install_fake_lit_modules():
        module = integration.import_lit_server()
    assert module.__name__ == "lit_nlp.server"
    assert getattr(module, "Server", None) is not None
...
def test_parse_cli_args_returns_expected_defaults() -> None:
    ...
    host, port, open_browser = integration.parse_cli_args([])
