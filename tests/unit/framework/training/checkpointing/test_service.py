...
from ml_playground.framework.training.checkpointing import service
...
def _with_sample_out_dir(
    shared: MetadataConfig, sample_out_dir: Path
) -> MetadataConfig:
...
def test_save_checkpoint_invokes_manager(tmp_path: Path) -> None:
    ...
    shared = create_metadata_config(
        ...
        experiment="unit",
        mkdir=False,
        train_out_dir=cfg_latest.runtime.out_dir,
        sample_out_dir=cfg_latest.runtime.out_dir,
    )
...
def test_save_checkpoint_uses_override(tmp_path: Path) -> None:
    ...
    ema = make_ema()
    ema.shadow = {"ema": torch.tensor([1.0])}
    ...
    assert "ema" in payload["checkpoint"].ema
    assert torch.equal(payload["checkpoint"].ema["ema"], torch.tensor([1.0]))
...
def test_propagate_metadata_dedup_when_dirs_equal(tmp_path: Path) -> None:
    """propagate_metadata should not duplicate copy when train and sample dirs are the same."""
    dataset_dir = tmp_path / "dataset"
    dataset_dir.mkdir()
    meta_src = dataset_dir / "meta.pkl"
    meta_src.write_bytes(b"meta")
    ...
    def counting_copy(src: Path, dst: Path) -> None:
        copy_calls.append((src, dst))
        import shutil

        shutil.copy2(src, dst)

    service.propagate_metadata(cfg, shared, logger=LoggerStub(), copy_fn=counting_copy)
