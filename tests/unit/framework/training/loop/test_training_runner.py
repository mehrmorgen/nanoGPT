...
import ml_playground.framework.training.loop.runner as runner_mod
...
from tests.unit.framework.training._helpers import (
    LoggerStub,
    SimpleBatchesStub,
...
def _build_deps(
    *,
    ...
    def init_batches(cfg: TrainerConfig, shared: MetadataConfig) -> BatchProvider:
        del cfg, shared
    ...
    def create_manager(cfg: TrainerConfig, shared: MetadataConfig) -> CheckpointManager:
        del cfg, shared
...
def test_train_entrypoint_uses_dependencies(
    ...
    summary = run_training(TrainingPlan(config=cfg, deps=deps))
    iters, best = summary.iterations, summary.best_loss
...
def test_checkpoint_model_args() -> None:
    ...
    checkpoint_data: CheckpointPayload = {
        "model_args": cast(dict[str, object], {"n_layer": 1}),
        "config": cast(dict[str, object], {"model_args": {"n_layer": 2}}),
        "optimizer": {},
        "iter_num": 0,
        "best_val_loss": 0.0,
        "model": {},
    ...
    fallback_model_args: dict[str, object] = {"n_layer": 2}
    fallback_data: CheckpointPayload = {
        "config": cast(dict[str, object], {"model_args": fallback_model_args}),
        "optimizer": {},
        "iter_num": 0,
        "best_val_loss": 0.0,
        "model": {},
...
def test_trainer_property_setters(
    trainer_harness: TrainerHarness,
) -> None:
    """Lines 184, 200, 206: exercise scaler, writer, and vectorize_impl setters."""
    fixture = trainer_harness.build(
        evaluation=default_evaluation(),
        max_iters=0,
    )
    ...
    # scaler setter (line 184)
    new_scaler = runner_mod.GradScaler(device="cpu", enabled=False)
    trainer.scaler = new_scaler
    assert trainer.scaler is new_scaler

    # writer setter (line 200)
    new_writer = TensorboardWriterStub()
    trainer.writer = cast(TensorboardWriter, new_writer)
    assert trainer.writer is new_writer

    # set_vectorize_impl (line 206)
    def fake_vmap(fn: object) -> object:
        return fn

    trainer.set_vectorize_impl(cast(VectorizeFn, fake_vmap))
    assert trainer._vectorize_impl is fake_vmap  # type: ignore[reportPrivateUsage]


def test_default_trainer_dependencies_with_vectorize_arg() -> None:
    """Lines 102-104: _vectorize() returns the provided vectorize arg directly."""

    def my_vmap(fn: object) -> object:
        return fn

    deps = runner_mod.default_trainer_dependencies(
        vectorize=cast(VectorizeFn, my_vmap),
    )
    result = deps.vectorize()
    assert result is my_vmap


def test_default_trainer_dependencies_vectorize_fallback() -> None:
    """Line 104: _vectorize() falls back to getattr(torch, 'vmap', None)."""
    deps = runner_mod.default_trainer_dependencies()
    result = deps.vectorize()
    # Result should be torch.vmap if available, or None
    expected = getattr(torch, "vmap", None)
    assert result is expected


def test_train_step_full_path_grad_accum_clip_ema(
    trainer_harness: TrainerHarness,
) -> None:
    """Lines 353, 358-359, 368: full _train_step with grad_accum > 1, grad_clip, and EMA.

    Exercises the actual _train_step code path (no override) through run().
    """
    ...
    # Set up a real EMA so line 368 is exercised
    counting_ema = _CountingEMA(cast(GPT, trainer.model))
    trainer.ema = counting_ema

    # Run WITHOUT train_step_override so the real _train_step path executes
    it, _best = trainer.run()
    assert it >= 1
    assert counting_ema.calls >= 1


def test_train_step_vmap_raises_when_vectorize_impl_none(
    trainer_harness: TrainerHarness,
) -> None:
    """Line 425: _train_step_vmap raises RuntimeError when _vectorize_impl is None."""
    fixture = trainer_harness.build(
        evaluation={-1: {"train": 0.5, "val": 0.4}},
        grad_accum_steps=2,
        max_iters=0,
    ...
    trainer = fixture.trainer
    trainer.set_vectorize_impl(None)

    train_step_vmap = cast(
        Callable[[torch.Tensor, torch.Tensor, int], torch.Tensor],
        getattr(trainer, "_train_step_vmap"),
    )

    inputs = torch.zeros((2, 4))
    targets = torch.zeros((2, 4))
    with pytest.raises(RuntimeError, match="Vectorization requested but unavailable"):
        train_step_vmap(inputs, targets, 2)


def test_trainer_run_without_writer(
    trainer_harness: TrainerHarness,
) -> None:
    """Branch 333->336: writer is None at cleanup, so writer.close() is skipped."""
    fixture = trainer_harness.build(
        evaluation=default_evaluation(),
        max_iters=1,
    )
    trainer = fixture.trainer
    # Set writer to None so the close branch is skipped
    trainer.writer = None
...
def test_trainer_skips_eval_when_interval_not_met(
    trainer_harness: TrainerHarness,
) -> None:
    """Branch 225->257: eval block is skipped when iter_num % eval_interval != 0."""
    fixture = trainer_harness.build(
        evaluation={-1: {"train": 0.5, "val": 0.4}},
        max_iters=2,
        eval_interval=100,  # eval only every 100 iters, so iter 1 skips eval
    )
    it, _best = fixture.run()
    assert it >= 1


def test_trainer_skips_log_when_interval_not_met(
    trainer_harness: TrainerHarness,
) -> None:
    """Branch 263->294: log block is skipped when iter_num % log_interval != 0."""
    fixture = trainer_harness.build(
        evaluation={-1: {"train": 0.5, "val": 0.4}},
        max_iters=2,
        eval_interval=100,
        log_interval=100,  # log only every 100 iters, so iter 1 skips logging
        tensorboard_mode="log",
    )
    it, _best = fixture.run()
    assert it >= 1
