...
def test_gpt_init_raises_when_vocab_size_zero() -> None:
    """GPT should raise ValueError when vocab_size is 0."""
    from ml_playground.framework.models.core.config import GPTConfig as RawGPTConfig

    cfg = RawGPTConfig(block_size=4, vocab_size=0, n_layer=1, n_head=1, n_embd=4)
    with pytest.raises(ValueError, match="vocab_size must be set"):
        GPT(cfg, logging.getLogger(__name__))


def test_gpt_init_without_logger() -> None:
    """GPT should skip the parameter log when logger is None."""
    cfg = ModelConfig(
...
def test_get_num_params_including_embeddings() -> None:
    """get_num_params(non_embedding=False) should include embedding parameters."""
    model = _make_model()
    total = model.get_num_params(non_embedding=False)
    without_emb = model.get_num_params(non_embedding=True)
    assert total > without_emb


def test_forward_raises_when_seq_exceeds_block_size() -> None:
    """forward should raise ValueError when sequence length exceeds block_size."""
    model = _make_model()
    model.eval()
    seq_len = model.config.block_size + 1
    idx = torch.randint(0, model.config.vocab_size, (1, seq_len))
