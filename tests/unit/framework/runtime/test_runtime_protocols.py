...
def test_metadata_config_like_requires_paths(tmp_path: Path) -> None:
    class MetadataCfg:
        def __init__(self) -> None:
    ...
    assert isinstance(MetadataCfg(), MetadataConfigLike)  # type: ignore[arg-type]
...
def test_runtime_checkable_rejects_non_conforming() -> None:
    ...
    assert not isinstance(NotMetadata(), MetadataConfigLike)
