...
class CharTokenizerTestHarness(CharTokenizer):
    ...
    def invalidate_lookup_array(self) -> None:
        if getattr(self, "_itos_array", None) is not None:
...
def test_tiktoken_tokenizer_properties_with_fake_module() -> None:
    ...
    class FakeEncoder:
        def __init__(self) -> None:
            self.n_vocab = 3
            self._mergeable_ranks = {b"a": 1, b"b": 2, b"c": 3}
    ...
    tk = TiktokenTokenizer(loader=cast(Any, lambda: FakeTiktokenModule()))
    assert tk.name == "tiktoken"
    assert tk.vocab_size == 3
    assert tk.decode(tk.encode("hi")) == "ab"
    v = tk.vocab
    # Mapping with expected keys
    assert getattr(v, "__getitem__", None) is not None and "a" in v and v["a"] == 1
...
def test_tiktoken_tokenizer_handles_missing_mergeable_ranks() -> None:
    ...
    tk = TiktokenTokenizer(loader=cast(Any, lambda: Module()))
...
def test_tiktoken_tokenizer_import_error_is_propagated() -> None:
    ...
    with pytest.raises(ImportError) as exc:
        TiktokenTokenizer(loader=cast(Any, loader))
...
def test_create_tokenizer_char_with_invalid_vocab_type() -> None:
    """create_tokenizer should raise TypeError for char tokenizer with non-mapping vocab."""
    with pytest.raises(TypeError) as exc:
        create_tokenizer("char", vocab=[1, 2, 3])  # List instead of dict
...
def test_tiktoken_tokenizer_vocab_without_mergeable_ranks() -> None:
    ...
    tk = TiktokenTokenizer(loader=cast(Any, lambda: FakeModule()))
    assert tk.vocab == MappingProxyType({})
