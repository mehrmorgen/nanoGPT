"""Targeted unit tests for `ml_playground.framework.data_pipeline.sampling.batches`."""
...
from ml_playground.framework.data_pipeline.sampling.batches import (
    SimpleBatches,
    sample_batch,
)
...
def test_simple_batches_random_sampler_path_covered(tmp_path: Path) -> None:
    """`SimpleBatches.get_batch` should use random sampler when configured."""
    ...
    # Call get_batch to ensure random sampler path is exercised
...
def test_simple_batches_raises_when_train_missing(tmp_path: Path) -> None:
    """SimpleBatches should raise FileNotFoundError when train.bin is missing."""

    arr = np.arange(8, dtype=np.uint16)
    ddir = _dataset_dir(tmp_path, train=arr, val=arr)
    # Remove train.bin
    (ddir / "train.bin").unlink()

    cfg = DataConfig(batch_size=1, block_size=4, sampler="random")
    with pytest.raises(FileNotFoundError, match="Training data not found"):
        SimpleBatches(cfg, device=CPU, dataset_dir=ddir)


def test_simple_batches_raises_when_val_missing(tmp_path: Path) -> None:
    """SimpleBatches should raise FileNotFoundError when val.bin is missing."""
...
def test_simple_batches_meta_non_dict_dtype_ignored(tmp_path: Path) -> None:
    """Meta with non-dict dtype should be ignored, defaulting to uint16."""

    arr = np.arange(8, dtype=np.uint16)
    # Create meta with dtype as a non-dict object
    ddir = _dataset_dir(tmp_path, train=arr, val=arr, meta={"dtype": 123})

    cfg = DataConfig(batch_size=1, block_size=4, sampler="random")
    batches = SimpleBatches(cfg, device=CPU, dataset_dir=ddir)

    # Should default to uint16 when dtype is not a string
    assert batches.train.arr.dtype == np.uint16


def test_simple_batches_meta_unknown_dtype_string_ignored(tmp_path: Path) -> None:
    """Meta with unknown dtype string should be ignored, defaulting to uint16."""

    arr = np.arange(8, dtype=np.uint16)
    # Create meta with dtype as an unknown string
    ddir = _dataset_dir(tmp_path, train=arr, val=arr, meta={"dtype": "uint64"})

    cfg = DataConfig(batch_size=1, block_size=4, sampler="random")
    batches = SimpleBatches(cfg, device=CPU, dataset_dir=ddir)

    # Should default to uint16 when dtype is an unknown string
    assert batches.train.arr.dtype == np.uint16
