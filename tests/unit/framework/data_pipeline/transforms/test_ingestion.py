...
from ml_playground.framework.data_pipeline.transforms.ingestion import (
    stream_csv_column,
    stream_jsonl,
    stream_text_lines,
...
def test_stream_text_lines_yields_lines(tmp_path: Path) -> None:
    """Streaming text lines yields lines without newlines."""
    path = tmp_path / "lines.txt"
    path.write_text("first\nsecond\nthird", encoding="utf-8")

    assert list(stream_text_lines(path)) == ["first", "second", "third"]


def test_stream_csv_column_missing_column_error(tmp_path: Path) -> None:
    """Streaming CSV raises when requested column is missing from header."""
    path = tmp_path / "rows.csv"
    path.write_text("text,other\nvalue1,value2\n", encoding="utf-8")

    with pytest.raises(DataError):
        list(stream_csv_column(path, column="missing"))


def test_stream_csv_column_yields_values(tmp_path: Path) -> None:
    """Streaming CSV yields values from the requested column."""
    path = tmp_path / "rows.csv"
    path.write_text("text,other\nvalue1,value2\nvalue3,value4\n", encoding="utf-8")

    assert list(stream_csv_column(path, column="text")) == ["value1", "value3"]


def test_validate_jsonl_record_rejects_meta_tokens_not_list() -> None:
    """JSONL records reject meta_tokens that are not lists."""
    with pytest.raises(DataError):
        validate_jsonl_record({"text": "hi", "meta_tokens": "not-a-list"})


def test_validate_jsonl_record_valid_meta_tokens() -> None:
    """JSONL records accept valid meta_tokens list."""
    validate_jsonl_record({"text": "hi", "meta_tokens": ["token1", "token2"]})


def test_stream_jsonl_non_existent_path_returns_empty(tmp_path: Path) -> None:
    """Streaming JSONL returns empty iterator for non-existent paths."""
