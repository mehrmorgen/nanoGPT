...
from ml_playground.framework.core.tokenizer import CharTokenizer, WordTokenizer
...
def test_prepare_with_tokenizer_word_vocab_rebuild() -> None:
    ...
    assert train_arr.dtype == np.uint16 and val_arr.dtype == np.uint16
    assert getattr(rebuilt, "encode", None) is not None
    assert getattr(rebuilt, "decode", None) is not None
...
def test_pipeline_run_uses_tokenizer_factory(tmp_path: Path) -> None:
    ...
    shared = create_metadata_config(tmp_path, experiment="unit")
...
def test_pipeline_cfg_property_returns_config(tmp_path: Path) -> None:
    """Test pipeline cfg property returns config."""
    text_path = tmp_path / "raw.txt"
    text_path.write_text("abba", encoding="utf-8")
    shared = create_metadata_config(tmp_path, experiment="unit")
    cfg = PreparerConfig(
...
def test_pipeline_shared_property_returns_shared_config(tmp_path: Path) -> None:
    """Test pipeline shared property returns shared config."""
    text_path = tmp_path / "raw.txt"
    text_path.write_text("abba", encoding="utf-8")
    shared = create_metadata_config(tmp_path, experiment="unit")
    cfg = PreparerConfig(
...
def test_pipeline_output_snapshot_returns_file_states(tmp_path: Path) -> None:
    """Test pipeline output_snapshot returns file states."""
    text_path = tmp_path / "raw.txt"
    text_path.write_text("abba", encoding="utf-8")
    shared = create_metadata_config(tmp_path, experiment="unit")
    cfg = PreparerConfig(
    ...
    file1 = shared.dataset_dir / "file1.bin"
    file2 = shared.dataset_dir / "file2.bin"
    file1.write_bytes(b"data1")
    file2.write_bytes(b"data2")

    snapshot = pipeline.output_snapshot([file1, file2])
    assert str(file1) in snapshot
    assert str(file2) in snapshot


def test_pipeline_split_validation_non_numeric_type(tmp_path: Path) -> None:
    """Test pipeline split validation with non-numeric type."""
    shared = create_metadata_config(tmp_path, experiment="unit")
    cfg = PreparerConfig(
        raw_text_path=tmp_path / "text.txt",
        tokenizer_type="char",
        extras={"split": ["invalid"]},  # List is not int, float, or str
    )
    pipeline = create_pipeline(cfg, shared)
    with pytest.raises(DataError, match="Invalid split ratio"):
        pipeline.prepare_from_text("abba", DummyTok())
...
def test_pipeline_split_validation_valid_ratios(tmp_path: Path) -> None:
    """Test pipeline split validation accepts valid ratios."""
    shared = create_metadata_config(tmp_path, experiment="unit")
    ...
    assert outcome.metadata["val_tokens"] == 10

    # Test with 1.0 (edge case)
    cfg = PreparerConfig(
        raw_text_path=text_path,
        tokenizer_type="char",
        extras={"split": 1.0},
    ...
    pipeline = create_pipeline(cfg, shared)
    outcome = pipeline.run()
    assert outcome.metadata["train_tokens"] == 10
    assert outcome.metadata["val_tokens"] == 0

    # Test with 0.5 (middle case)
    cfg = PreparerConfig(
        raw_text_path=text_path,
        tokenizer_type="char",
        extras={"split": 0.5},
    ...
    pipeline = create_pipeline(cfg, shared)
    outcome = pipeline.run()
    assert outcome.metadata["train_tokens"] == 5
    assert outcome.metadata["val_tokens"] == 5
