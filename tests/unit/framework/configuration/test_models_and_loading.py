...
class MetadataConfigTestHarness(config_models.MetadataConfig):
    """Test harness exposing MetadataConfig protected validators."""
    ...
    def resolve_paths(
        ...
        del context
        descriptor = getattr(config_models.MetadataConfig, "_resolve_metadata_paths")
        validator = descriptor.__get__(  # pyright: ignore[reportPrivateUsage]
            None, config_models.MetadataConfig
        )
        return validator(data)  # pyright: ignore[reportGeneralTypeIssues]
...
def test_full_loader_roundtrip(tmp_path: Path) -> None:
    ...
    assert exp.sampling is not None
    assert isinstance(exp.training.runtime.out_dir, Path)
    assert isinstance(exp.metadata.dataset_dir, Path)
...
def test_load_train_config_requires_mapping(tmp_path: Path) -> None:
    ...
    config.write_text("training = 'value'\n")

    default_config = tmp_path / "default.toml"
    default_config.write_text("")

    with pytest.raises(TypeError, match="\\[training\\] section"):
...
def test_full_loader_nested_unknown_keys_in_sample_raise(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError):
        config_loading.load_full_experiment_config(cfg_path, tmp_path, "exp")
...
def test_full_loader_nested_unknown_keys_raise(tmp_path: Path) -> None:
    ...
    text = text.replace("[training.model]", "[training.model]\nunknown_key = 123")
    cfg_path.write_text(text)
    with pytest.raises(ValidationError):
        config_loading.load_full_experiment_config(cfg_path, tmp_path, "exp")


def test_full_loader_rejects_deprecated_sections(tmp_path: Path) -> None:
    """Legacy train/sample/shared sections should fail fast."""
    cfg_path = tmp_path / "deprecated.toml"
    cfg_path.write_text("[train]\nfoo = 1\n[shared]\nbar = 2\n", encoding="utf-8")
    with pytest.raises(
        ValueError, match="Deprecated config sections are no longer supported"
    ):
        config_loading.load_full_experiment_config(cfg_path, tmp_path, "exp")
...
def test_load_experiment_toml_strict_sections(tmp_path: Path) -> None:
    ...
    assert exp.sampling.runtime is not None


def test_experiment_config_resolves_metadata_and_section_paths(tmp_path: Path) -> None:
    """Test experiment config resolves metadata and section paths."""
    ...
    assert exp.metadata.config_path == cfg_path.resolve()
    assert exp.metadata.project_home == config_dir.parent.resolve()
    assert exp.metadata.dataset_dir == (config_dir / "prep_dataset").resolve()
    assert exp.metadata.train_out_dir == (config_dir / "train_out").resolve()
    assert exp.metadata.sample_out_dir == (config_dir / "sample_out").resolve()
    assert exp.prepare.raw_dir == (config_dir / "raw").resolve()
    assert exp.prepare.raw_text_path == (config_dir / "texts" / "input.txt").resolve()
    assert exp.training.runtime.out_dir == (config_dir / "train_out").resolve()
    assert exp.sampling.runtime.out_dir == (config_dir / "sample_out").resolve()
    assert exp.metadata.dataset_dir == (config_dir / "prep_dataset").resolve()
...
def test_merge_mappings_numeric_replacements() -> None:
    ...
    base = {"a": {"x": 1, "y": -2}, "b": 10}
    override = {"a": {"x": 3}, "b": 0}
    out = cast(Any, merge_mappings(base, override))
    assert out["a"]["x"] == 3
    assert out["a"]["y"] == -2
    assert out["b"] == 0
...
def test_trainer_resolves_relative_runtime_out_dir(tmp_path: Path) -> None:
    ...
    exp = config_loading.load_full_experiment_config(cfg_path, tmp_path, "exp")
    assert isinstance(exp.training.runtime.out_dir, Path)
    assert str(exp.training.runtime.out_dir).endswith("out/rel_train")
...
def test_sampler_resolves_relative_runtime_out_dir(tmp_path: Path) -> None:
    ...
    cfg_path.write_text(cfg_text, encoding="utf-8")
    exp = config_loading.load_full_experiment_config(cfg_path, tmp_path, "exp")
    assert isinstance(exp.sampling.runtime.out_dir, Path)
    assert str(exp.sampling.runtime.out_dir).endswith("out/rel_sample")


def test_experiment_config_metadata_path_coercions(tmp_path: Path) -> None:
    """Test experiment config metadata path coercions."""
    ds_dir = Path("./data/metadata")
    ...
    assert isinstance(exp.metadata.dataset_dir, Path)
    assert isinstance(exp.metadata.train_out_dir, Path)
    assert isinstance(exp.metadata.sample_out_dir, Path)
    assert exp.metadata.train_out_dir.is_absolute()
    assert exp.metadata.sample_out_dir.is_absolute()
    assert str(exp.metadata.train_out_dir).endswith(str(out_dir))
    assert str(exp.metadata.sample_out_dir).endswith(str(out_dir))
...
def test_preparerconfig_path_coercion_and_resolve(tmp_path: Path) -> None:
    ...
    config = PreparerConfig.model_validate({"raw_dir": tmp_path / "raw"})
...
def test_config_canonical_exports() -> None:
    ...
    from ml_playground.framework.configuration import models
    from ml_playground.framework.configuration import loading

    missing = object()
    assert getattr(models, "TrainerConfig", missing) is not missing
    assert getattr(models, "SamplerConfig", missing) is not missing
    assert getattr(models, "DataConfig", missing) is not missing
    assert getattr(models, "RuntimeConfig", missing) is not missing
    assert getattr(loading, "load_full_experiment_config", missing) is not missing
...
def test_full_loader_incomplete_sample_config(tmp_path: Path) -> None:
    ...
    toml_text = toml_text.replace("[sampling.sample]", "# Missing sample.sample")
...
def test_full_loader_sample_missing_runtime_section(tmp_path: Path) -> None:
    ...
    toml_text = toml_text.replace("[sampling.runtime]", "# Missing [sampling.runtime]")
...
def test_cli_adapters_load_and_validate(tmp_path: Path) -> None:
    ...
    assert exp.metadata.experiment == "exp"
...
def test_cli_adapters_prerequisites(tmp_path: Path) -> None:
    ...
    runtime_meta_dir = exp.metadata.sample_out_dir / exp.metadata.experiment
...
def test_experiment_config_resolve_paths_from_dict(tmp_path: Path) -> None:
    ...
    data = {
        ...
        "training": {"runtime": {"out_dir": "train_rel"}},
        "sampling": {"runtime": {"out_dir": "sample_rel"}},
        "metadata": {
    ...
    metadata = cast(Mapping[str, Any], result["metadata"])
    assert isinstance(metadata["project_home"], Path)
    assert isinstance(metadata["dataset_dir"], Path)
    assert isinstance(metadata["train_out_dir"], Path)
    assert isinstance(metadata["sample_out_dir"], Path)
    ...
    assert isinstance(result["training"]["runtime"]["out_dir"], Path)
    assert isinstance(result["sampling"]["runtime"]["out_dir"], Path)
...
def test_experiment_config_resolve_paths_with_namespace(tmp_path: Path) -> None:
    ...
    cfg_path = tmp_path / "cfg.toml"
    metadata_ns = SimpleNamespace(config_path=cfg_path)
    data = {"metadata": metadata_ns}
    # Should not raise or mutate namespace for non-dict metadata data
    result = cast(Mapping[str, Any], ExperimentConfigTestHarness.resolve_paths(data))
    assert result["metadata"] is metadata_ns


def test_metadata_config_resolves_relative_paths(tmp_path: Path) -> None:
    """Test metadata config resolves relative paths."""
    cfg_path = tmp_path / "cfg.toml"
    data = {
        "config_path": cfg_path,
        "project_home": "proj",
    ...
    resolved = MetadataConfigTestHarness.resolve_paths(data.copy())
...
def test_metadata_config_resolve_paths_invalid_config_path() -> None:
    """Test metadata config resolve paths invalid config path."""
    data = {"config_path": object(), "project_home": "rel", "dataset_dir": 123}
    resolved = cast(
        Mapping[str, Any], MetadataConfigTestHarness.resolve_paths(data.copy())
    )
...
def test_resolve_if_relative_edge_cases(tmp_path: Path) -> None:
    ...
    from ml_playground.framework.configuration.utils import resolve_if_relative
...
def test_coerce_path_edge_cases() -> None:
    ...
    from ml_playground.framework.configuration.utils import coerce_path
...
def test_experiment_config_path_resolution_edge_cases(tmp_path: Path) -> None:
    ...
    # Test with metadata data that has config_path but no other path fields - covers line 271
    data: Mapping[str, Any] = {
        "metadata": {"config_path": cfg_path, "experiment": "test"},
        ...
        "sampling": {"runtime": {"out_dir": "sample_out"}, "sample": {}},
    }
...
def test_cross_field_validator_coverage() -> None:
    ...
    from ml_playground.framework.configuration.models import RuntimeConfig, LRSchedule
...
def test_normalize_runtime_out_dir_edge_cases(tmp_path: Path) -> None:
    ...
    data: Mapping[str, Any] = {
        ...
        "sampling": {"sample": {}},  # No runtime section
    }
...
def test_metadata_config_path_coercion_edge_cases(tmp_path: Path) -> None:
    """Test MetadataConfig path coercion edge cases."""
    cfg_path = tmp_path / "cfg.toml"
    ...
    result = cast(Mapping[str, Any], MetadataConfigTestHarness.resolve_paths(data))
...
def test_lr_schedule_validator_warmup_exceeds_decay() -> None:
    ...
    from ml_playground.framework.configuration.models import LRSchedule
