...
import ml_playground.tools.agentic.agentic as agentic_module
from ml_playground.tools.agentic.agentic import (
    QualityBatchResults,
    TestBatchResults as _TestBatchResults,
)
...
def swap_attr(target, name: str, value):
    ...
    missing = object()
    original = target_dict.get(name, missing)
...
class TestBatchReview:
    ...
    def test_batch_review_handles_quality_failure(
        ...
        class DeterministicAgenticTools(agentic_module.AgenticTools):
            ...
            def _run_test_batch(self) -> _TestBatchResults:
                return cast(
                    _TestBatchResults,
                    {
                        "overall": {
...
class TestBatchValidate:
    ...
    def test_batch_validate_minimal(
        ...
        result = cast(_TestBatchResults, output_data["test_results"])
        assert "overall_success" in output_data
...
class TestWorkflowStatus:
    ...
    def test_assess_readiness_reports_blockers(
        ...
        blocking = cast(list[str], readiness.get("blocking_issues", []))

        assert readiness["ready_for_merge"] is False
        assert "Quality checks failing (2 issues)" in blocking
        assert "Test failures detected" in blocking
        assert "Uncommitted changes present" in blocking
