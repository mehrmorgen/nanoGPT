...
class TestEnvironmentToolsInit:
    ...
    def test_init(
        ...
        """Test initialization."""
        tools_any = cast(object, environment_tools)
        assert getattr(tools_any, "config") == config
        assert getattr(tools_any, "root_path") == root_path
        assert getattr(tools_any, "cache_dir") == root_path / ".cache"
        assert getattr(tools_any, "venv_path") == root_path / ".venv"
        assert getattr(tools_any, "pkg_name") == "ml_playground"
...
class TestSetup:
    ...
    def test_setup_clear_uses_backup_strategy(
        ...
        venv_path = cast(Path, getattr(cast(object, environment_tools), "venv_path"))
...
class TestClean:
    ...
    def test_clean_removes_cache_and_pycache(
        ...
        cache_pytest = (
            cast(Path, getattr(cast(object, environment_tools), "cache_dir")) / "pytest"
        )
        cache_pytest.mkdir(parents=True, exist_ok=True)
        (cache_pytest / "state.bin").write_bytes(b"binary")

        pycache_dir = (
            cast(Path, getattr(cast(object, environment_tools), "root_path"))
            / "src"
            / "__pycache__"
        )
