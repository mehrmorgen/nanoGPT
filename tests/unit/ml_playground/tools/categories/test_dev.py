...
def _root_path(tools: dev.DevTools) -> Path:
    return cast(Path, getattr(cast(object, tools), "root_path"))
...
def test_tools_cli_main_sets_dry_run_env(tmp_path: Path) -> None:
    ...
    cli_main.main(learning_mode=False, verbosity=1, dry_run=True, project_root=None)
