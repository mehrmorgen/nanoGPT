...
def test_bundestag_char_preparer_uses_base_dir_from_init(tmp_path: Path) -> None:
    """prepare should use the base_dir provided in __init__ when no override is provided."""
    mock_module_dir = tmp_path / "mock_module"
    mock_module_dir.mkdir()
    mock_file = mock_module_dir / "preparer.py"
    mock_file.touch()

    # Pass the fake module directory to the preparer via __init__ (DI)
    preparer = BundestagCharPreparer(base_dir=mock_module_dir)

    exp_dir = mock_module_dir
    ds_dir = exp_dir / "datasets"
