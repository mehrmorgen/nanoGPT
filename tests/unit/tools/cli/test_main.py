...
import ml_playground.tools.cli.commands.learn as learn_commands
...
tools_cli = import_module("ml_playground.tools.cli.main")
...
class TestCLIBasics:
    ...
    def test_learn_best_practices_success(self) -> None:
        ...
        assert result.exit_code == 0
        assert "🤖 CI Best Practices" in result.stdout
    ...
    def test_analysis_lit_executes_command_body(self, tmp_path: Path) -> None:
        ...
        assert "LIT server stopped" in result.stdout
    ...
    def test_analysis_sample_quality_executes_command_body(
        ...
        with (
            ...
            override_attr(analysis_commands, "format_analysis", fake_format_analysis),
        ):
            runner = CliRunner()
            result = runner.invoke(
                tools_cli.app,
...
class TestCLIErrorBranches:
    ...
    def test_test_unit_handles_tool_execution_error(self) -> None:
        ...
        def make_failing_testing_factory(
            _cfg: ToolsConfig, _root: Path | None
        ) -> FailingTestingTools:
            return FailingTestingTools()

        deps = _deps(testing_factory=make_failing_testing_factory)
        with override_tools_dependencies(deps):
    ...
    def test_env_tensorboard_handles_tool_execution_error(self) -> None:
        ...
        def make_failing_environment_factory(
            _cfg: ToolsConfig, _root: Path | None
        ) -> FailingEnvironmentTools:
            return FailingEnvironmentTools()

        deps = _deps(environment_factory=make_failing_environment_factory)
        with override_tools_dependencies(deps):
            logdir = Path(".").resolve()
    ...
    def test_ci_quality_gate_handles_tool_execution_error(self) -> None:
        ...
        def make_failing_ci_factory(
            _cfg: ToolsConfig, _root: Path | None
        ) -> FailingCITools:
            return FailingCITools()

        deps = _deps(ci_factory=make_failing_ci_factory)
        with override_tools_dependencies(deps):
...
class TestCLIIntegration:
    ...
    def test_ci_quality_ci_local_success(self) -> None:
        ...
        class StubCITools:
            ...
            def quality_ci_local(
                self, args: list[str], bind_caches: bool
            ) -> ToolResult:
    ...
    def test_dev_review_list_success(self) -> None:
        ...
        stub = StubDevTools()
        deps = _deps(dev_factory=lambda _cfg, _root: stub)
        with override_tools_dependencies(deps):
            tools_cli.state.config = ToolsConfig()
            tools_cli.state.project_root = Path.cwd()
            result = self.runner.invoke(
...
class TestDevCommandFailures:
    ...
    def test_dev_review_list_handles_tool_error(self) -> None:
        ...
        deps = _deps(
            dev_factory=lambda _cfg, _root: FailingDevTools(),
        )
    ...
    def test_dev_batch_review_handles_configuration_error(self) -> None:
        ...
        class FailingDevTools:
            def batch_review(self, *_: object, **__: object) -> ToolResult:
                # Configuration errors usually bubble up as generic application errors
                # or are caught by specific command wrappers. For this test, assume
                # untrapped exceptions result in exit 1 and stderr message.
        ...
        assert "bad config" in (cli_result.stderr or cli_result.stdout)
    ...
    def test_dev_batch_review_success_propagates_result(self) -> None:
        ...
        stub = StubDevTools()
        deps = _deps(
            dev_factory=lambda _cfg, _root: stub,
        )
        ...
        assert "format=text" in cli_result.stdout
    ...
    def test_dev_cleanup_handles_tool_error(self) -> None:
        ...
        deps = _deps(dev_factory=lambda _cfg, _root: FailingDevTools())
...
class TestLearnCommands:
    ...
    def test_learn_explain_unknown_command_in_category(self) -> None:
        ...
        assert "Available commands" in (result.stderr or "")
        assert "lint" in result.stdout
...
class TestToolFactories:
    ...
    def test_get_quality_tools_uses_cached_config(self) -> None:
        ...
        class FakeQualityTools:
            def __init__(self, config: ToolsConfig, root_path: Path | None) -> None:
    ...
    def test_tool_factories_create_expected_classes(
        ...
        def agentic_factory(_cfg: ToolsConfig, root: Path) -> FactoryStub:
            return FactoryStub(root_path=root)

        def dev_factory(_cfg: ToolsConfig, root: Path | None = None) -> FactoryStub:
            return FactoryStub(root_path=root)
    ...
    def test_nested_factory_methods_via_default_dependencies(self) -> None:
        ...
        assert getattr(env_tools, "config", None) is not None
        ...
        assert getattr(ci_tools, "config", None) is not None
        ...
        assert getattr(dev_tools, "config", None) is not None
