...
class TestQualityGate:
    ...
    def test_quality_gate_success(self, ci_tools: CITools):
        ...
        with patch('ml_playground.tools.categories.ci.run_uv_command') as mock_run:
            ...
            operation_id = OperationId(namespace="tools", category="ci", command="quality-gate")
...
class TestQualityExt:
    ...
    def test_quality_ext_success(self, ci_tools: CITools):
        ...
        with patch.object(ci_tools, 'quality_gate') as mock_quality_gate, \
            ...
            mutation_id = OperationId(namespace="tools", category="ci", command="mutation-run")
            mutation_result = create_success_result(mutation_id, "Mutation testing passed")
    ...
    def test_quality_ext_quality_gate_failure(self, ci_tools: CITools):
        ...
        with patch.object(ci_tools, 'quality_gate') as mock_quality_gate:
            operation_id = OperationId(namespace="tools", category="ci", command="quality-gate")
            failure_result = create_failure_result(operation_id, 1, "", "Quality gate failed")
...
class TestMutationTesting:
    ...
    def test_mutation_summary(self, ci_tools: CITools):
        ...
        with patch('ml_playground.tools.categories.ci.run_uv_command') as mock_run:
            operation_id = OperationId(namespace="tools", category="ci", command="mutation-summary")
    ...
    def test_mutation_init_new_session(self, ci_tools: CITools):
        ...
        with patch('ml_playground.tools.categories.ci.run_uv_command') as mock_run:
            operation_id = OperationId(namespace="tools", category="ci", command="mutation-init")
    ...
    def test_mutation_init_existing_session(self, ci_tools: CITools):
        ...
        with patch('ml_playground.tools.categories.ci.run_uv_command') as mock_run:
            operation_id = OperationId(namespace="tools", category="ci", command="mutation-init")
            failure_result = create_failure_result(operation_id, 1, "", "Session exists")
            mock_run.return_value = failure_result
    ...
    def test_mutation_exec_success(self, ci_tools: CITools):
        ...
        with patch('ml_playground.tools.categories.ci.run_uv_command') as mock_run:
            operation_id = OperationId(namespace="tools", category="ci", command="mutation-exec")
            success_result = create_success_result(operation_id, "Mutation execution completed")
            mock_run.return_value = success_result
    ...
    def test_mutation_report(self, ci_tools: CITools):
        ...
        with patch('ml_playground.tools.categories.ci.run_uv_command') as mock_run:
            operation_id = OperationId(namespace="tools", category="ci", command="mutation-report")
            success_result = create_success_result(operation_id, "Mutation report generated")
            mock_run.return_value = success_result
    ...
    def test_mutation_run_full_pipeline(self, ci_tools: CITools):
        ...
        with patch.object(ci_tools, 'mutation_reset') as mock_reset, \
            ...
            operation_id = OperationId(namespace="tools", category="ci", command="mutation-run")
    ...
    def test_mutation_run_step_failure(self, ci_tools: CITools):
        ...
        with patch.object(ci_tools, 'mutation_reset') as mock_reset, \
            ...
            reset_id = OperationId(namespace="tools", category="ci", command="mutation-reset")
            success_result = create_success_result(reset_id, "Reset completed")
            mock_reset.return_value = success_result
            
            summary_id = OperationId(namespace="tools", category="ci", command="mutation-summary")
...
class TestQualityCILocal:
    ...
    def test_quality_ci_local_with_cache_binding(self, ci_tools: CITools):
        ...
        with patch('ml_playground.tools.categories.ci.subprocess.run') as mock_run:
            mock_result = type('MockResult', (), {
                'returncode': 0,
                'stdout': 'CI completed with caches',
                'stderr': ''
            })()
            mock_run.return_value = mock_result
