...
def test_cleanup_ignored_tracked_removes_files(
    ...
    assert runner.calls[0].get("command", [])[:4] == ["git", "ls-files", "-c", "-i"]
...
def test_cleanup_ignored_tracked_stop_after_failed_removal(
    ...
    calls = runner.calls
    assert calls[0].get("command", [])[:4] == ["git", "ls-files", "-c", "-i"]
...
def test_tools_cli_main_sets_dry_run_env(tmp_path: Path) -> None:
    ...
    cli = import_module("ml_playground.tools.cli.main")
...
def test_review_infer_repo_fallback_and_failure(
    ...
    tools, runner = dev_tools
    mod = ReviewModule(runner, tools._root_path)
...
def test_apply_filters_unreplied_and_unresolved():
    from ml_playground.tools.dev.review import ReviewModule, ReviewComment, ReviewThread
    from tests.unit.tools.fakes import FakeSubprocessRunner
    ...
    t1 = ReviewThread(id="", url="", is_resolved=True, comments=[])
    # Thread 2: unresolved but has viewer comment -> filtered when unreplied=True
    c_viewer = ReviewComment(author="", viewer_did_author=True, body="", url="", id="")
    t2 = ReviewThread(id="t2", url="url2", is_resolved=False, comments=[c_viewer])
    # Thread 3: unresolved and no viewer comment -> kept
    c_other = ReviewComment(author="", viewer_did_author=False, body="", url="", id="")
    t3 = ReviewThread(id="t3", url="url3", is_resolved=False, comments=[c_other])
...
def test_comment_lookup_maps_id_url_suffix_and_dbid():
    ...
    th = ReviewThread(id="t1", url="https://x/y#disc", is_resolved=False, comments=[cm])
    fetch = ReviewFetchResult(threads=[th], viewer=None)
...
def test_render_threads_handles_empty_comment_body() -> None:
    ...
    comment = ReviewComment(author="me", viewer_did_author=True, body="", url="", id="")
    thread = ReviewThread(id="t", url="u", is_resolved=False, comments=[comment])
    lines = mod.render_threads(
        [thread],
        apply_filters=lambda x, **k: cast(list[ReviewThread], x),  # type: ignore[no-any-return]
        unreplied=False,
        unresolved=False,
        viewer="me",
    )
...
def test_cleanup_ignored_tracked_removes_files_alt(tmp_path: Path) -> None:
    ...
    assert runner.calls[0]["command"][:4] == ["git", "ls-files", "-c", "-i"]
...
def test_kill_port_exception_path_returns_failure(
    ...
    """Test kill_port exception handling via dependency injection."""
    tools, runner = dev_tools

    def raising(port: int) -> list[int]:
        raise OSError("network error")

    tools._pids_by_port = raising
    tools._kill_pid = lambda _: True
...
def test_comment_lookup_resolves_id_url_anchor_and_dbid() -> None:
    ...
    th = ReviewThread(id="t1", url="u", is_resolved=False, comments=[cm])
    fetch = ReviewFetchResult(threads=[th], viewer="me")
...
def test_render_threads_multiline_comment_formats_continuations() -> None:
    ...
    body = "first line\nsecond line\nthird"
    cm = ReviewComment(
        author="bob", viewer_did_author=False, body=body, url="u/c", id="c1"
    )
    thread = ReviewThread(id="t1", url="u", is_resolved=False, comments=[cm])
    lines = mod.render_threads(
        [thread],
        apply_filters=lambda x, **k: cast(list[ReviewThread], x),  # type: ignore[no-any-return]
        unreplied=False,
...
def test_review_list_graphql_failure_raises(
    ...
    assert result.success is False
    assert "Failed to list review comments" in (result.stderr or "")
...
def test_comment_lookup_empty_fetch_returns_empty() -> None:
    from ml_playground.tools.dev.review import ReviewFetchResult, ReviewModule
    from tests.unit.tools.fakes import FakeSubprocessRunner

    runner = FakeSubprocessRunner()
    mod = ReviewModule(runner, Path.cwd())
    fetch = ReviewFetchResult(threads=[], viewer=None)
