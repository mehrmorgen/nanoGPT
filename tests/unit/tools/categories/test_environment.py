...
import ml_playground.tools.environment.environment as environment_module
...
class TestEnvironmentToolsInit:
    ...
    def test_init(
        ...
        """Test initialization."""
        assert environment_tools._config == config
        assert environment_tools._root_path == root_path
        assert environment_tools._cache_dir == root_path / ".cache"
        assert environment_tools._venv_path == root_path / ".venv"
        assert environment_tools._pkg_name == "ml_playground"
...
class TestSetup:
    ...
    def test_setup_success(
        ...
        assert sync_command[:4] == ["uv", "sync", "--group", "all"]

    def test_setup_clear_uses_backup_strategy(
        self,
        environment_tools: environment_module.EnvironmentTools,
        subprocess_runner: FakeSubprocessRunner,
        root_path: Path,
    ) -> None:
        """Ensure `clear=True` backs up and then replaces an existing virtual environment."""
        venv_path = environment_tools._venv_path
        ...
        assert result.success is True
        assert "Backed up existing virtual environment" in result.stdout
        assert "Removed virtual environment backup" in result.stdout
        assert not venv_path.exists()

    def test_setup_clear_restores_backup_on_failure(
        self,
        environment_tools: environment_module.EnvironmentTools,
        subprocess_runner: FakeSubprocessRunner,
    ) -> None:
        """If clear setup fails, previous environment should be restored."""
        venv_path = environment_tools._venv_path
        venv_path.mkdir(parents=True, exist_ok=True)
        marker = venv_path / "placeholder.txt"
        marker.write_text("keep me", encoding="utf-8")

        operation_id = OperationId(namespace="tools", category="env", command="setup")
        subprocess_runner.set_results(
            [create_failure_result(operation_id, exit_code=2, stderr="venv failed")]
        )
...
class TestSync:
    ...
    def test_sync_all_groups(
        ...
        assert len(subprocess_runner.calls) == 1
        command = subprocess_runner.calls[0]["command"]
        assert command[:4] == ["uv", "sync", "--group", "all"]
...
class TestClean:
    ...
    def test_clean_removes_cache_and_pycache(
        ...
        cache_pytest = environment_tools._cache_dir / "pytest"
        cache_pytest.mkdir(parents=True, exist_ok=True)
        (cache_pytest / "state.bin").write_bytes(b"binary")

        pycache_dir = environment_tools._root_path / "src" / "__pycache__"
...
class TestTensorboard:
    ...
    def test_tensorboard_missing_dir_raises(
        ...
        with pytest.raises(ToolExecutionError):
            environment_tools.tensorboard([], environment_tools._root_path / "missing")
