...
class TestTestingToolsInit:
    def test_init(
        ...
        tools_any = cast(object, testing_tools)
        assert getattr(tools_any, "_config") == config
        assert getattr(tools_any, "_root_path") == root_path
        assert getattr(tools_any, "_cache_dir") == root_path / ".cache"
...
def test_ensure_coverage_data_combines_fragments(
    ...
    assert result is None
    assert any(
        cast(list[str], call["args"])[:2] == ["coverage", "combine"]
        for call in runner.uv_calls
    )
    ...
    def test_mutation_summary_reports_configuration(
        ...
        def fake_load_config(path: str) -> dict[str, object]:
            ...
            return {
                ...
                "modules": {
                    "paths": ["ml_playground.framework.core", "ml_playground.tools"]
                },
            }
        ...
        def fake_find_modules(cfg: object) -> list[str]:
            return ["ml_playground.framework.core", "ml_playground.tools"]
...
def test_budget_report_reads_config_and_cache(
    testing_tools: testing_module.TestingTools, root_path: Path
) -> None:
...
""".lstrip(),
    ...
    cache_path = root_path / ".cache" / "test_budgets.json"
    cache_path.parent.mkdir(parents=True, exist_ok=True)
    cache_path.write_text(
        json.dumps({"integration_seconds": 4.0, "e2e_seconds": 12.0}),
        encoding="utf-8",
    )

    result = testing_tools.budget_report([])
