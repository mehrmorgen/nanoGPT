...
class TestQualityGate:
    ...
    def test_quality_gate_success(
        ...
        verify_result = create_success_result(operation_id, "env ok")
        precommit_result = create_success_result(operation_id, "All checks passed")
        fake_runner.set_results([verify_result, precommit_result])
    ...
    def test_precommit_stdout_is_appended_to_summary(
        ...
        verify_result = create_success_result(operation_id, "env ok")
        precommit_result = create_success_result(operation_id, "pre ok")
        fake_runner.set_results([verify_result, precommit_result])
    ...
    def test_quality_gate_recovers_environment(
        self, ci_tools: CITools, fake_runner: FakeSubprocessRunner
    ) -> None:
        operation_id = OperationId(
            namespace="tools", category="ci", command="quality-gate"
        )
        verify_fail = create_failure_result(operation_id, 1, "", "import failed")
        setup_venv = create_success_result(operation_id, "venv ok")
        setup_sync = create_success_result(operation_id, "sync ok")
        verify_ok = create_success_result(operation_id, "env ok")
        precommit_ok = create_success_result(operation_id, "pre ok")
        fake_runner.set_results(
            [verify_fail, setup_venv, setup_sync, verify_ok, precommit_ok]
        )

        result = ci_tools.quality_gate([])

        assert result.success is True
        assert "- environment: RECOVERED" in (result.stdout or "")
        assert "Environment steps:" in (result.stdout or "")
        assert "verify: FAIL" in (result.stdout or "")
        assert "setup: PASS" in (result.stdout or "")
        assert len(fake_runner.calls) == 5

    def test_quality_gate_fails_when_setup_fails(
        self, ci_tools: CITools, fake_runner: FakeSubprocessRunner
    ) -> None:
        operation_id = OperationId(
            namespace="tools", category="ci", command="quality-gate"
        ...
        verify_fail = create_failure_result(operation_id, 1, "", "import failed")
        setup_fail = create_failure_result(operation_id, 2, "", "setup failed")
        fake_runner.set_results([verify_fail, setup_fail])

        result = ci_tools.quality_gate([])

        assert result.success is False
        assert result.exit_code == 2
        assert "- environment: FAIL" in (result.stdout or "")
        assert "Pre-commit skipped due to environment failure" in (result.stdout or "")
        assert len(fake_runner.calls) == 2

    def test_quality_gate_surfaces_verify_remediation(
        self, ci_tools: CITools, fake_runner: FakeSubprocessRunner
    ) -> None:
        operation_id = OperationId(
            namespace="tools", category="ci", command="quality-gate"
        )
        verify_fail = create_failure_result(operation_id, 1, "", "missing: yamlfix")
        setup_fail = create_failure_result(operation_id, 2, "", "setup failed")
        fake_runner.set_results([verify_fail, setup_fail])
        ...
        assert result.success is False
        assert "Verify errors:\nmissing: yamlfix" in (result.stdout or "")
