...
from ml_playground.tools.cli.commands import quality as quality_commands
...
tools_cli = importlib.import_module("ml_playground.tools.cli.main")


@contextmanager
def swap_attr(target: object, name: str, value: object) -> Iterator[None]:
    original: object = getattr(target, name)
...
@contextmanager
def swap_attr_by_path(path: str, value: object) -> Iterator[None]:
    module_path, attr = path.rsplit(".", 1)
    module = importlib.import_module(module_path)
    original: object = getattr(module, attr)
...
def _deps_with_loader(
    loader: Callable[[Path | None], ToolsConfig],
) -> ToolsDependencies:
    base = default_tools_dependencies()
    return ToolsDependencies(
        load_config=loader,
        quality_factory=base.quality_factory,
        testing_factory=base.testing_factory,
        environment_factory=base.environment_factory,
        ci_factory=base.ci_factory,
        agentic_factory=base.agentic_factory,
        dev_factory=base.dev_factory,
    )
...
class TestCLIErrorBranches:
    ...
    def test_main_handles_configuration_error(self) -> None:
        """`load_tools._config` raising `ToolConfigurationError` should exit with code 1."""
        ...
        with override_tools_dependencies(_deps_with_loader(fake_load_tools_config)):
    ...
    def test_env_tensorboard_handles_tool_execution_error(self) -> None:
        ...
        with swap_attr(
            ...
            logdir = Path(".").resolve()
            result = self.runner.invoke(  # type: ignore
                tools_cli.app,
    ...
    def test_cli_handles_unexpected_config_error(self) -> None:
        ...
        with override_tools_dependencies(_deps_with_loader(boom)):
            result = self.runner.invoke(tools_cli.app, ["version"])
...
class TestLearnCommands:
    ...
    def test_learn_commands_overview(self):
        ...
        assert "Environment Tools" in result.stdout
    ...
    def test_learn_best_practices_overview(self):
        ...
        assert "General Workflow" in result.stdout
    ...
    def test_learn_best_practices_category_specific(self):
        ...
        assert result.exit_code == 0
        assert "🧹 Quality Best Practices" in result.stdout
        assert "Quality Tools" in result.stdout
...
class TestQualityCommands:
    ...
    def test_quality_lint_success(self) -> None:
        ...
        with swap_attr(quality_commands, "get_quality_tools", lambda: stub):
            result = self.runner.invoke(  # type: ignore
    ...
    def test_quality_format_failure(self) -> None:
        ...
        with swap_attr(
            quality_commands, "get_quality_tools", lambda: StubQualityTools()
        ):
            result = self.runner.invoke(
                tools_cli.app,
                ["quality", "format"],
...
class TestAgenticCommands:
    ...
    def test_agentic_guidelines_uses_learning_state(self) -> None:
    ...
    def test_agentic_batch_review_handles_tool_error(self) -> None:
...
class TestToolFactories:
    ...
    def test_get_quality_tools_uses_cached_config(self) -> None:
        ...
        with swap_attr_by_path(
            "ml_playground.tools.quality.quality.QualityTools", FakeQualityTools
        ):
    ...
    @pytest.mark.parametrize(  # type: ignore[reportAny]
        ...
        [
            (
                "get_testing_tools",
                "ml_playground.tools.testing.testing.TestingTools",
                True,
            ),
            (
                "get_environment_tools",
                "ml_playground.tools.environment.environment.EnvironmentTools",
                True,
            ),
            ("get_ci_tools", "ml_playground.tools.ci.ci.CITools", True),
            (
                "get_agentic_tools",
                "ml_playground.tools.agentic.agentic.AgenticTools",
                True,
            ),
            ("get_dev_tools", "ml_playground.tools.dev.dev.DevTools", False),
        ],
    )
...
class TestInvokeTests:
        ...
        with swap_attr(cli_helpers, "get_testing_tools", lambda: stub):
            ...
            try:
                ...
                test_commands._invoke_tests(
                    "tests/unit", pattern="selected", extra_args=["-q"]
                )
            finally:
    ...
    def test_invoke_tests_handles_unknown_suite(self) -> None:
        with swap_attr(
            cli_helpers, "get_testing_tools", lambda: self.StubTestingTools()
        ):
            with pytest.raises(typer.Exit) as exc:
                test_commands._invoke_tests(
                    "tests/unknown", pattern=None, extra_args=[]
                )
