...
def test_run_coverage_report_does_not_duplicate_executed_command(
    ...
    def fake_ensure(**_: object) -> tuple[list[str], list[str], dict[str, str]]:
        return [pre], [], env
...
def test_run_coverage_threshold_raises_when_coverage_file_missing(
    ...
    def fake_ensure(**_: Any) -> tuple[list[str], list[str], dict[str, str]]:
        return [], [], {}
...
def test_run_coverage_map_reports_undercovered_files(
    ...
    gap_file = tmp_path / "src" / "ml_playground" / "framework" / "core" / "gap.py"
    gap_file.parent.mkdir(parents=True, exist_ok=True)
    gap_file.write_text("value = 1", encoding="utf-8")
    ...
    assert "src/ml_playground/framework/core/gap.py" in result.stdout
    assert "expected=tests/unit + tests/property" in result.stdout


def test_run_coverage_map_reports_no_gaps(config: ToolsConfig, tmp_path: Path) -> None:
    runner = CoverageRunner()
    _prepare_cached_coverage(tmp_path)
    ...
    assert "Coverage map: no files below 100% coverage." in result.stdout
