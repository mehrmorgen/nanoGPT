...
class TestLearnCommands:
    ...
    def test_learn_commands_invalid_category(self):
        ...
        assert "Unknown category 'invalid'" in result.stderr or "Unknown category 'invalid'" in result.stdout
    ...
    def test_learn_explain_invalid_format(self):
        ...
        assert result.exit_code == 1
        # Error messages go to stderr in Typer
        assert "Command must be in format 'category.command'" in result.stderr or "Command must be in format 'category.command'" in result.stdout
    ...
    def test_learn_explain_verbosity_levels(self):
        ...
        result = self.runner.invoke(app, ["learn", "explain", "test.unit", "--verbosity", "0"])
        assert result.exit_code == 0
        
        # Test standard verbosity
        result = self.runner.invoke(app, ["learn", "explain", "test.unit", "--verbosity", "1"])
        assert result.exit_code == 0
        
        # Test comprehensive verbosity
        result = self.runner.invoke(app, ["learn", "explain", "test.unit", "--verbosity", "2"])
        assert result.exit_code == 0
    ...
    def test_learn_best_practices_category_specific(self):
        ...
        result = self.runner.invoke(app, ["learn", "best-practices", "--category", "quality"])
    ...
    def test_learn_best_practices_invalid_category(self):
        ...
        result = self.runner.invoke(app, ["learn", "best-practices", "--category", "invalid"])
    ...
    def test_learn_best_practices_verbosity_levels(self):
        ...
        # Test minimal verbosity
        result = self.runner.invoke(app, ["learn", "best-practices", "--verbosity", "0"])
        assert result.exit_code == 0
        
        # Test comprehensive verbosity
        result = self.runner.invoke(app, ["learn", "best-practices", "--verbosity", "2"])
        assert result.exit_code == 0
