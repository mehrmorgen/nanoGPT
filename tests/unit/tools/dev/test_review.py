...
def test_review_module_factory_public(tmp_path: Path) -> None:
    ...
    # It expects certain members to exist if it uses it.
    # Since the internal code uses getattr(review, "fetch_review_threads"),
    # we just need to make sure the mock has it.
    mock_review.fetch_review_threads.return_value = SimpleNamespace(
        threads=[], viewer="me"
    )
    mock_review._infer_repo.return_value = ("o", "r")

    tools = DevTools(root_path=tmp_path, review_module_factory=lambda: mock_review)
    tools.review_list(pr_number=1)

    assert mock_review.fetch_review_threads.called
...
def test_batch_review_public(tmp_path: Path) -> None:
    ...
    with patch("ml_playground.tools.dev.batch_review.run_batch_review") as mock_run:
        mock_run.return_value = ToolResult.create(
            success=True,
            exit_code=0,
            namespace="tools",
            category="dev",
...
def test_kill_port_psutil_fallback(monkeypatch: Any, tmp_path: Path) -> None:
    ...
    def fake_process_iter(args: Any = None) -> list[FakeProc]:
        return [FakeProc(123, 5555)]

    monkeypatch.setattr(dev_module.psutil, "process_iter", fake_process_iter)
...
def test_kill_port_psutil_fallback_no_laddr(monkeypatch: Any, tmp_path: Path) -> None:
    ...
    def fake_process_iter_no_laddr(args: Any = None) -> list[FakeProcNoLaddr]:
        return [FakeProcNoLaddr()]

    monkeypatch.setattr(dev_module.psutil, "process_iter", fake_process_iter_no_laddr)

    tools = DevTools(root_path=tmp_path, pids_by_port=None, kill_pid=lambda pid: True)
    result = tools.kill_port(5555)
    assert result.success is True
