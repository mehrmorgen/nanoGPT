from __future__ import annotations

from pathlib import Path

import numpy as np
import numpy.typing as npt
import pytest
from ml_playground.runtime_cli.main import main


@pytest.fixture
def tmp_dataset(tmp_path: Path) -> Path:
    """Create the smallest dataset that still satisfies CLI expectations.

    The char pipeline requires enough tokens to cover a 64-token block size and to
    populate both `train.bin` and `val.bin`. A 512-token uint16 ramp meets those
    constraints while keeping disk writes tiny and deterministic.
    """
    ds = tmp_path / "dataset"
    ds.mkdir(parents=True, exist_ok=True)
    arr: npt.NDArray[np.uint16] = (
        np.arange(512, dtype=np.uint16) % np.uint16(256)
    ).astype("uint16")
    (ds / "train.bin").write_bytes(arr.tobytes())
    (ds / "val.bin").write_bytes(arr.tobytes())
    meta = {
        "meta_version": 1,
        "kind": "char",
        "tokenizer_type": "char",
        "dtype": "uint16",
        "train_tokens": len(arr),
        "val_tokens": len(arr),
        "stoi": {chr(i): i for i in range(256)},
        "itos": {i: chr(i) for i in range(256)},
    }
    import pickle

    with (ds / "meta.pkl").open("wb") as f:
        pickle.dump(meta, f)
    return ds


def _write_exp_config(tmp_dir: Path, out_dir: Path, dataset_dir: Path) -> Path:
    """Emit a tiny-but-realistic config that exercises training + sampling.

    The numeric knobs are intentionally at the minimum values that still trigger
    checkpoint rotation, metrics logging, and sampler compatibility:

    - Model: single layer/head (`n_layer=1`, `n_head=2`, `n_embd=64`) still matches
      the 256-token vocab and 64-token block without hitting validation errors.
    - Runtime: CPU-only, `max_iters=4`, `eval_interval=1`, and `keep.last/best = 1`
      prove rotation logic while finishing in <0.1s.
    - Sampling: reuse the same out_dir so the sample command consumes the fresh
      checkpoint instead of rebuilding artifacts.
    """
    # Ensure Windows paths are properly escaped for TOML
    dataset_dir_str = str(dataset_dir).replace("\\", "\\\\")
    out_dir_str = str(out_dir).replace("\\", "\\\\")
    cfg = f'''
[prepare]
dataset_dir = "{dataset_dir_str}"
tokenizer_type = "char"

[training.model]
n_layer = 1
n_head = 2
n_embd = 64
block_size = 64
dropout = 0.0
bias = false
vocab_size = 256

[training.hf_model]
model_name = "hf/model"
gradient_checkpointing = false
block_size = 64

[training.peft]
enabled = false
r = 1
lora_alpha = 1
lora_dropout = 0.0
bias = "none"
target_modules = []
extend_mlp_targets = false

[training.data]
train_bin = "train.bin"
val_bin = "val.bin"
batch_size = 4
block_size = 64
grad_accum_steps = 1

[training.optim]
learning_rate = 0.0005
weight_decay = 0.0
beta1 = 0.9
beta2 = 0.95
grad_clip = 0.0

[training.schedule]
decay_lr = true
warmup_iters = 1
lr_decay_iters = 2
min_lr = 0.00008

[training.runtime]
out_dir = "{out_dir_str}"
max_iters = 4
eval_interval = 1
eval_iters = 1
log_interval = 1
eval_only = false
device = "cpu"
dtype = "float32"
compile = false
ckpt_time_interval_minutes = 0

[training.runtime.checkpointing.keep]
last = 1
best = 1

[sampling.runtime]
out_dir = "{out_dir_str}"
device = "cpu"
dtype = "float32"
compile = false
eval_only = false
max_iters = 0
eval_interval = 1
eval_iters = 1
log_interval = 1

[sampling.sample]
start = "Hi"
num_samples = 1
max_new_tokens = 8
temperature = 0.9
top_k = 50
'''
    p = tmp_dir / "config.toml"
    p.write_text(cfg)
    return p


def test_train_bundestag_char_quick(tmp_path: Path, tmp_dataset: Path) -> None:
    # Use a unique subdirectory to avoid race conditions in parallel runs
    import uuid

    out_dir = tmp_path / f"out_train_{uuid.uuid4().hex[:8]}"
    out_dir.mkdir(parents=True, exist_ok=True)
    cfg = _write_exp_config(tmp_path, out_dir, tmp_dataset)
    main(["--exp-config", str(cfg), "train", "bundestag_char"])  # run quickly

    assert out_dir.exists(), f"out_dir {out_dir} does not exist after training"

    # Debug: list directory contents
    contents = list(out_dir.glob("*"))
    content_names = [p.name for p in contents]
    print(f"\nDEBUG: out_dir contents: {content_names}")

    # Check for expected rotated checkpoint artifacts
    last_ckpts = list(out_dir.glob("ckpt_last_*.pt"))
    assert last_ckpts, f"no last checkpoint found. Contents: {content_names}"

    best_ckpts = list(out_dir.glob("ckpt_best_*.pt"))
    assert best_ckpts, f"no best checkpoint found. Contents: {content_names}"

    # meta.pkl should be propagated for sampling
    assert (out_dir / "meta.pkl").exists(), (
        f"meta.pkl missing. Contents: {content_names}"
    )


def test_sample_bundestag_char_quick(tmp_path: Path, tmp_dataset: Path) -> None:
    out_dir = tmp_path / "out_sample"
    # Train small to produce a checkpoint
    train_cfg = _write_exp_config(tmp_path, out_dir, tmp_dataset)
    main(["--exp-config", str(train_cfg), "train", "bundestag_char"])
    # Sample with minimal config pointing to same out_dir
    main(["--exp-config", str(train_cfg), "sample", "bundestag_char"])
