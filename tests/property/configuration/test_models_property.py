...
def _make_runtime(**overrides: object) -> config_models.RuntimeConfig:
...
def test_trainer_data_block_size_validator(model_block: int, data_block: int) -> None:
    ...
    with pytest.raises(ValueError, match="train\\.data\\.block_size"):
...
def test_trainer_schedule_min_lr_validator() -> None:
    ...
    with pytest.raises(ValueError, match="train\\.schedule\\.min_lr"):
...
def test_trainer_warmup_validator() -> None:
    ...
    with pytest.raises(ValueError, match="train\\.schedule\\.warmup_iters"):
        _build_trainer_config(model_block=4, data_block=4, schedule=schedule)
...
@settings(max_examples=25, deadline=50, derandomize=True)
@given(names=st.tuples(_safe_name(), _safe_name(), _safe_name()))
...
@given(names=st.tuples(_safe_name(), st.just("dup"), st.just("dup")))
...
def test_peft_adapter_target_modules_coerce_tuple() -> None:
    ...
    truncated = config_models.TrainerConfig.PeftConfig._coerce_target_modules(peft_dict)
...
def test_shared_config_resolves_paths(tmp_path: Path) -> None:
    data = {
        "experiment": "demo",
...
def test_experiment_config_resolves_prepare_paths(tmp_path: Path) -> None:
    cfg = {
        ...
        "train": {"runtime": {"out_dir": "train"}},
        "sample": {"runtime": {"out_dir": "sample"}},
        "shared": {
            "experiment": "demo",
            "config_path": str(tmp_path / "config.toml"),
    ...
    resolved = config_models.ExperimentConfig._resolve_paths(cfg)
    project_home = resolved["shared"]["project_home"]
...
@settings(max_examples=15, deadline=50, derandomize=True)
@given(relative=st.text(alphabet=string.ascii_lowercase, min_size=1, max_size=6))
...
def test_data_config_declines_ngram_with_char_tokenizer() -> None:
    ...
    with pytest.raises(ValueError, match="train\\.data\\.ngram_size must be 1"):
...
def test_trainer_config_runtime_paths_resolve(tmp_path: Path) -> None:
    ...
    assert "mlruns.db" in trainer.runtime.mlflow_tracking_uri
...
def test_experiment_config_shared_paths_gain_prepare_dataset(tmp_path: Path) -> None:
    """ExperimentConfig hoists prepare dataset paths into shared config."""
    ...
    cfg = config_models.ExperimentConfig.model_validate(
        {
            ...
            "sample": {"runtime": {"out_dir": "sample"}, "sample": {}},
            "shared": shared,
        }
    )
    assert cfg.shared.dataset_dir.is_absolute()
    assert cfg.prepare.raw_dir.is_absolute()


def test_shared_config_resolves_relative_fields(tmp_path: Path) -> None:
    """SharedConfig resolves members relative to the config location."""
    ...
    resolved = config_models.SharedConfig._resolve_shared_paths(data)
    assert (
