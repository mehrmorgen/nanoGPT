...
def test_tool_result_creation_with_all_fields(
    ...
    # ToolResult always creates a LearningInfo, never None
...
@given(
    success=st.booleans(),
    exit_code=st.integers(min_value=0, max_value=255),
    namespace=st.text(min_size=1, max_size=10),
...
def test_tool_result_factory_method(
    ...
    result = ToolResult.create(
        success=success,
        exit_code=exit_code,
        namespace="ml",  # Must be "ml" or "tools"
        category=category,
        command=command,
    )
...
def test_operation_id_creation_and_equality(
    namespace: str, category: str, command: str
) -> None:
...
def test_learning_mode_engine_verbosity(verbosity: VerbosityLevel) -> None:
    ...
    # Should have explanations based on verbosity
    assert len(info.explanations) >= 0
