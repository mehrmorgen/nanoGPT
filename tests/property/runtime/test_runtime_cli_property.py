...
def _build_stub_dependencies(log: DependencyCallLog) -> runtime_cli.CLIDependencies:
    def load_experiment(experiment: str, exp_config: Path | None) -> SimpleNamespace:
        log.loaded.append((experiment, exp_config))
        logger = logging.getLogger("ml_playground.runtime.tests")
        ...
        train_cfg = SimpleNamespace(logger=logger, runtime=runtime_cfg)
        sample_cfg = SimpleNamespace(logger=logger, runtime=runtime_cfg)
...
def _override_dependencies(log: DependencyCallLog) -> Iterator[None]:
    ...
    with runtime_cli.override_cli_dependencies(deps):
        yield
...
def _invoke_runtime_cli(
    ...
    if log is None:
        return CLI_RUNNER.invoke(runtime_cli.app, list(raw_args))
