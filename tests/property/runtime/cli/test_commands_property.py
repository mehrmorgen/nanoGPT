...
def override_maps(draw: st.DrawFn) -> dict[str, Any]:
    ...
    if draw(st.booleans()):
        overrides["analysis_runner"] = lambda *args, **kwargs: SimpleNamespace(spec=ToolResult)
...
def test_select_override_value_finds_correct_key(overrides: dict[str, Any]) -> None:
    ...
    result = commands._select_override_value(overrides, "missing1", "test_key", "missing2")
...
@given(overrides=override_maps())
@settings(max_examples=10, deadline=None, derandomize=True)
def test_select_override_callable_filters_non_callables(overrides: dict[str, Any]) -> None:
...
def test_prepare_command_uses_overrides(
    ...
    try:
        ...
        setattr(commands_module, 'prepare_learning_context', 
...
@given(
    ...
    verbosity=st.sampled_from(list(VerbosityLevel)),
    overrides=override_maps(),
    host=st.text(min_size=1, max_size=15, alphabet="abcdefghijklmnopqrstuvwxyz0123456789"),
...
def test_command_functions_extract_dependencies() -> None:
    ...
    original_funcs['prepare_learning_context'] = commands_module.prepare_learning_context
