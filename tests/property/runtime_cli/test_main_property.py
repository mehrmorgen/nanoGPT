...
main_module: Any = importlib.import_module("ml_playground.runtime_cli.main")
...
@given(  # type: ignore[reportAny]
    argv=st.one_of(st.none(), argv_lists())
)
...
def test_main_returns_exit_code(argv: list[str] | None) -> None:
    ...
    original_get_command = main_module.get_command

    def _get_command(app: object) -> SimpleNamespace:
        _ = app
        return fake_cmd

    main_module.get_command = _get_command
    ...
    try:
        ...
        main_module.get_command = original_get_command
...
def test_main_entry_exception_handling(
    ...
    assert callable(main_module.main_entry)
...
def test_create_default_cli_dependencies_structure() -> None:
    """Test that create_default_cli_dependencies returns properly structured dependencies."""
    deps = main_module.create_default_cli_dependencies()

    assert deps.__class__.__name__ == "CLIDependencies"
...
def test_create_default_cli_dependencies_consistency(call_count: int) -> None:
    """Test that create_default_cli_dependencies returns consistent results."""
    deps_list = [
        main_module.create_default_cli_dependencies() for _ in range(call_count)
    ]
...
def test_main_entry_echo_function_default() -> None:
    ...
    try:
        main_module.app = mock_runner  # type: ignore[assignment]
        main_module.main_entry()
    finally:
        main_module.app = original_app  # type: ignore[assignment]


@given(  # type: ignore[reportAny]
    call_count=st.integers(min_value=1, max_value=3)
)
...
def test_all_exports_available() -> None:
    ...
    for name in main_module.__all__:
        missing = object()
        assert getattr(main_module, name, missing) is not missing, (
            f"Export {name} not found in module"
        )
