...
def test_configure_and_get_dependencies_preserve_tag(tag: str) -> None:
    ...
    bootstrap.configure_cli_dependencies(lambda: _make_dependencies(tag))
...
def test_nested_override_context_restores_correctly(tag1: str, tag2: str) -> None:
    ...
    bootstrap.configure_cli_dependencies(lambda: baseline)

    with bootstrap.override_cli_dependencies(override1):
        assert bootstrap.get_cli_dependencies() is override1

        with bootstrap.override_cli_dependencies(override2):
            assert bootstrap.get_cli_dependencies() is override2

        assert bootstrap.get_cli_dependencies() is override1

    assert bootstrap.get_cli_dependencies() is baseline
...
def test_dependency_functions_receive_correct_parameters(
    ...
    bootstrap.configure_cli_dependencies(lambda: deps)
...
def test_context_manager_exception_restores_dependencies() -> None:
    ...
    try:
        with bootstrap.override_cli_dependencies(override):
            assert bootstrap.get_cli_dependencies() is override
...
def test_all_dependency_functions_are_callable(tag: str) -> None:
    ...
    assert getattr(exp_result, "tag", None) is not None

    deps.ensure_train_prerequisites(None)
    deps.ensure_sample_prerequisites(None)

    prepare_result = deps.run_prepare("cmd", None, Path("/tmp"), None, deps, None)
    assert getattr(prepare_result, "tag", None) is not None

    train_result = deps.run_train("cmd", None, Path("/tmp"), None, deps, None)
    assert getattr(train_result, "tag", None) is not None

    sample_result = deps.run_sample("cmd", None, Path("/tmp"), None, deps, None)
    assert getattr(sample_result, "tag", None) is not None
