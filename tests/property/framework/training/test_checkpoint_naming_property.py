...
@given(  # type: ignore[reportAny]
    iter_num=st.integers(min_value=0, max_value=1_000_000)
)
...
@settings(max_examples=30, deadline=None, derandomize=True)
@given(  # type: ignore[reportAny]
    counter_value=st.integers(min_value=0, max_value=1_000_000)
)
...
def test_checkpoint_naming_strict_rejects_deprecated_files() -> None:
    """Strict naming rejects deprecated checkpoint filenames for domain counters."""
    with TemporaryDirectory() as tmp_dir:
        out_dir = Path(tmp_dir)
...
def test_checkpoint_naming_strict_rejects_deprecated_best_files() -> None:
    """Strict naming rejects deprecated best-checkpoint filenames."""
    with TemporaryDirectory() as tmp_dir:
        out_dir = Path(tmp_dir)
