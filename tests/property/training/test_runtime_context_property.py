...
def test_runtime_context_when_device_then_sets_device_type(
    ...
    runtime = RuntimeConfig(out_dir=tmp_path, device=device, dtype=dtype, seed=seed)
...
def test_runtime_context_when_cuda_available_then_seeds_cuda(
    tmp_path: Path, seed: int
) -> None:
    """Runtime context when cuda available then seeds cuda."""
    runtime = RuntimeConfig(out_dir=tmp_path, device="cuda", seed=seed)
...
def test_runtime_context_when_logger_configured_then_sets_formatter(
    ...
    runtime = RuntimeConfig(out_dir=tmp_path, device=device)
