...
@given(args=st.lists(st.text(min_size=1, max_size=6), max_size=3))
...
def test_format_invokes_ruff_format(args: list[str], tmp_path: Path) -> None:
    ...
    assert result.success is True
    call = runner.calls[-1]
    expected = ["ruff", "format"] if not args else ["ruff", *args]
    assert call.args[: len(expected)] == expected[: len(call.args)]


@settings(max_examples=20, deadline=None, derandomize=True)
@given(args=st.lists(st.text(min_size=1, max_size=4), max_size=2))
