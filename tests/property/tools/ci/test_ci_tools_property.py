...
def test_quality_gate_executes_precommit(extra_args: list[str], tmp_path: Path) -> None:
    ...
    assert any(call.kind == "uv" and "pre-commit" in call.args for call in runner.calls)


@settings(max_examples=10, deadline=None, derandomize=True)
@given(st.just(()))
def test_coverage_badge_writes_svg(_: tuple[()], tmp_path: Path) -> None:
