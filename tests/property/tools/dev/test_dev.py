...
@st.composite
def st_thread(draw: st.DrawFn) -> _Thread:
    url = draw(
        st.text(
            min_size=1,
            alphabet=st.characters(blacklist_categories=("Cs",), min_codepoint=32),
    ...
    return _Thread(url=url, is_resolved=is_resolved, comments=comments)


@st.composite
def st_fetch_result(draw: st.DrawFn) -> _FetchResult:
...
def test_comment_lookup_invariants(fetch: _FetchResult) -> None:
...
def test_load_replies_valid_json(tmp_path: Path, replies_dict: Dict[str, str]) -> None:
    ...
    from ml_playground.tools.dev.dev import _load_replies
    ...
    # Act
    loaded = _load_replies(replies_file)
