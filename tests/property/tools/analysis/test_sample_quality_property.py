...
def test_extract_header_property(lines: list[str]) -> None:
    ...
    missing = object()
    assert getattr(header, "speaker", missing) is not missing
    assert getattr(header, "topic", missing) is not missing
    assert getattr(header, "year", missing) is not missing
...
@given(  # type: ignore[reportAny]
    lines_strategy, st.integers(min_value=1, max_value=10)
)
...
@given(  # type: ignore[reportAny]
    text_strategy, st.integers(min_value=1, max_value=5)
)
