...
GROUP_PATHS, COMMAND_PATHS = _collect_command_paths(tools_cli_main.app)
...
def _install_deterministic_runner() -> Iterator[DeterministicRunner]:
    ...
    for module in modules:
        if getattr(module, "_default_runner", None) is not None:
            object.__setattr__(module, "_default_runner", deterministic)
    ...
    try:
        ...
        for module, runner in original_module_runners.items():
            if runner is not None:
                object.__setattr__(module, "_default_runner", runner)
...
def test_commands_accept_sample_arguments(command: tuple[str, ...]) -> None:
    extra = SINGLE_VALUE_COMMANDS[cast(tuple[str, str], command)]
