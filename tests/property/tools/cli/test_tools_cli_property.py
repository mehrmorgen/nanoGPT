...
GROUP_PATHS, COMMAND_PATHS = _collect_command_paths(cli_main.app)
...
CATEGORIES = sorted(list(COMMAND_INFO.keys()))
...
CATEGORY_COMMAND_MAP = {
    category: sorted(list(info["commands"].keys()))
    for category, info in COMMAND_INFO.items()
}
...
@given(  # type: ignore[reportAny]
    flags=GLOBAL_FLAGS_STRATEGY, group_path=GROUP_PREFIX_STRATEGY
)
...
@given(  # type: ignore[reportAny]
    flags=GLOBAL_FLAGS_STRATEGY, bad_bool=_NON_BOOL_TEXT
)
@settings(max_examples=30, deadline=None, derandomize=True)
...
@given(  # type: ignore[reportAny]
    flags=GLOBAL_FLAGS_STRATEGY, missing_name=_REL_NAME_TEXT
)
@settings(max_examples=30, deadline=None, derandomize=True)
...
@given(  # type: ignore[reportAny]
    flags=GLOBAL_FLAGS_STRATEGY, filename=_REL_NAME_TEXT
)
@settings(max_examples=30, deadline=None, derandomize=True)
...
@given(  # type: ignore[reportAny]
    flags=GLOBAL_FLAGS_STRATEGY, bad_port=_NON_INT_TEXT
)
...
@given(  # type: ignore[reportAny]
    flags=GLOBAL_FLAGS_STRATEGY, category=INVALID_CATEGORY_STRATEGY
)
...
@given(  # type: ignore[reportAny]
    flags=GLOBAL_FLAGS_STRATEGY, detailed=st.booleans()
)
...
@given(  # type: ignore[reportAny]
    flags=GLOBAL_FLAGS_STRATEGY, category=CATEGORY_STRATEGY
)
...
@given(  # type: ignore[reportAny]
    flags=GLOBAL_FLAGS_STRATEGY, target=VALID_EXPLAIN_STRATEGY
)
...
@given(  # type: ignore[reportAny]
    flags=GLOBAL_FLAGS_STRATEGY, bad_value=st.text(min_size=1, max_size=10)
)
...
@given(  # type: ignore[reportAny]
    flags=GLOBAL_FLAGS_STRATEGY, bad_value=_NON_INT_TEXT
)
...
@given(  # type: ignore[reportAny]
    command=COMMAND_EXECUTION_STRATEGY, flags=GLOBAL_FLAGS_STRATEGY
)
...
@given(  # type: ignore[reportAny]
    command=st.sampled_from(list(SINGLE_VALUE_COMMANDS.keys()))
)
@settings(max_examples=20, deadline=None, derandomize=True)
...
def test_main_entry_invokes_cli_app(_: None) -> None:
    ...
    with ExitStack() as stack:
        _stack_override_attr(stack, cli_main, "app", fake_app)
...
def test_main_entry_handles_keyboard_interrupt(_: None) -> None:
    ...
    with ExitStack() as stack:
        _stack_override_attr(stack, cli_main, "app", fake_app)
        _stack_override_attr(stack, typer, "echo", fake_echo)
...
@given(  # type: ignore[reportAny]
    reason=st.text(min_size=1, max_size=25)
)
