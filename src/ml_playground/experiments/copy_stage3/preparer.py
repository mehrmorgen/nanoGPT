...
class CopyStage3Preparer(_PreparerProto):
    """Prepare deterministic Stage 3 data with SOS and EOS boundaries."""
    ...
    def prepare(self, cfg: PreparerConfig) -> PrepareReport:  # type: ignore[override]
        ...
        total_tokens = total_sequences * (payload_length + 2)
        ...
        if _artifacts_look_valid(outputs, stage="copy_stage3", expected_tokens=expected_tokens, total_tokens=total_tokens):
            return PrepareReport(
                created_files=tuple(),
                updated_files=tuple(),
                skipped_files=tuple(outputs),
                messages=(f"[copy_stage3] dataset already prepared at {ds_dir}; skipping.",),
            )

        pre = snapshot_file_states(outputs)
        data = _build_stage3_stream(
            symbols=symbols,
        ...
        meta["stage"] = "copy_stage3"
        meta["task"] = "copy_with_boundaries"
        ...
        return PrepareReport(
            ...
            messages=(f"[copy_stage3] prepared deterministic sos/eos dataset at {ds_dir}",),
        )
...
def _resolve_symbols(cfg: PreparerConfig, extras: Mapping[str, object]) -> tuple[str, str]:
    ...
    if isinstance(raw_text_path, Path):
        ...
        if len(unique) != 2:
            raise ValueError("copy_stage3 raw_text_path must contain exactly two unique symbols")
    ...
    if len(unique) != 2:
        raise ValueError("copy_stage3 requires two distinct payload symbols")
    return (unique[0], unique[1])
...
def _build_stage3_stream(
    *,
    ...
    payload = "".join(symbols[idx % 2] for idx in range(payload_length))
    sample = f"{sos_symbol}{payload}{eos_symbol}"
