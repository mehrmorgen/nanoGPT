...
class CopyStage2Preparer(_PreparerProto):
    """Prepare deterministic Stage 2 data with explicit EOS token."""
    ...
    def prepare(self, cfg: PreparerConfig) -> PrepareReport:  # type: ignore[override]
        ...
        if eos_symbol in symbols:
            raise ValueError("eos_symbol must differ from payload symbols")

        payload_length = _coerce_positive_int(extras.get("payload_length"), default=8)
        total_sequences = _coerce_positive_int(extras.get("total_sequences"), default=128)
        total_tokens = total_sequences * (payload_length + 1)

        ds_dir.mkdir(parents=True, exist_ok=True)
        outputs = [ds_dir / "train.bin", ds_dir / "val.bin", ds_dir / "meta.pkl"]

        expected_tokens = tuple(sorted((symbols[0], symbols[1], eos_symbol)))
        if _artifacts_look_valid(outputs, stage="copy_stage2", expected_tokens=expected_tokens, total_tokens=total_tokens):
            return PrepareReport(
                created_files=tuple(),
                updated_files=tuple(),
                skipped_files=tuple(outputs),
                messages=(f"[copy_stage2] dataset already prepared at {ds_dir}; skipping.",),
            )

        pre = snapshot_file_states(outputs)
        data = _build_stage2_stream(
            symbols=symbols,
        ...
        meta["stage"] = "copy_stage2"
        meta["task"] = "copy_with_eos"
        ...
        return PrepareReport(
            ...
            messages=(f"[copy_stage2] prepared deterministic eos dataset at {ds_dir}",),
        )
...
def _resolve_symbols(cfg: PreparerConfig, extras: Mapping[str, object]) -> tuple[str, str]:
    ...
    if isinstance(raw_text_path, Path):
        ...
        if len(unique) != 2:
            raise ValueError("copy_stage2 raw_text_path must contain exactly two unique symbols")
    ...
    if len(unique) != 2:
        raise ValueError("copy_stage2 requires two distinct payload symbols")
    return (unique[0], unique[1])
...
def _build_stage2_stream(
    *,
    ...
    payload_chars = [symbols[idx % 2] for idx in range(payload_length)]
    payload = "".join(payload_chars)
    sample = payload + eos_symbol
    return sample * total_sequences
