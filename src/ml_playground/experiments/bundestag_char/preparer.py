...
class BundestagCharPreparer(_PreparerProto):
    ...
    def prepare(self, cfg: PreparerConfig) -> PrepareReport:  # type: ignore[override]
        ...
        use_germaparl = dataset_source == "germaparl_tei" or (
            dataset_source == "auto"
            and not _any_candidates_exist(input_file_path, candidates)
        )
        if use_germaparl:
            processed = _prepare_germaparl_input(
                input_file_path,
                exp_dir,
                extras,
                progress=cast(Callable[[str], None], cfg.logger.info),
            )
            source_metadata = {
...
def _prepare_germaparl_input(
    ...
    *,
    ...
    if max_files is not None and max_files <= 0:
        raise DataError(
            f"Invalid germaparl_max_files: {max_files!r}",
            reason="germaparl_max_files must be a positive integer when provided",
            rationale="Preparing TEI input must process at least one XML file to avoid empty datasets",
        )
...
def _stream_encode_char_dataset(
    *,
    ...
    total_chars = 0
    next_scan_progress = _ENCODE_PROGRESS_CHARS
    if progress is not None:
        progress("[bundestag_char] char-encode scan pass started")
    for chunk in _iter_text_chunks(raw_path):
        total_chars += len(chunk)
        vocab_chars.update(chunk)
        if progress is not None and total_chars >= next_scan_progress:
            progress(
                f"[bundestag_char] char-encode scan pass: {total_chars:,} characters"
            )
            next_scan_progress += _ENCODE_PROGRESS_CHARS
    ...
    val_tokens = 0
    seen = 0
    next_write_progress = _ENCODE_PROGRESS_CHARS
    if progress is not None:
        progress(
            "[bundestag_char] char-encode write pass started: "
    ...
    with train_path.open("wb") as train_out, val_path.open("wb") as val_out:
        for chunk in _iter_text_chunks(raw_path):
            ...
            seen = chunk_end
            if progress is not None and seen >= next_write_progress:
                percent = 100.0 if total_chars == 0 else (100.0 * seen / total_chars)
                progress(
                    "[bundestag_char] char-encode write pass: "
                    f"{seen:,}/{total_chars:,} characters ({percent:.1f}%)"
                )
                next_write_progress += _ENCODE_PROGRESS_CHARS
    if progress is not None:
        progress(
            "[bundestag_char] char-encode complete: "
            f"train_tokens={train_tokens:,} val_tokens={val_tokens:,}"
        )
