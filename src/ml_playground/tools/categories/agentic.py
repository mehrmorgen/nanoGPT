...
class AgenticTools:
    ...
    def guidelines_setup(
        ...
        if guidelines_created:
            output += "\nFiles created:\n" + "\n".join(
                f"  - {file}" for file in guidelines_created
            )
...
"""
    ...
    def _determine_overall_status(
        self, quality_results: Dict[str, Any], test_results: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _run_validation_batch(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if config.get("coverage_required", False):
            coverage_results = self._check_coverage_requirements(
    ...
    def _format_validation_text_output(self, validation_results: Dict[str, Any]) -> str:
        ...
        if validation_results.get("quality_results"):
            output += "Quality Checks:\n"
            for check, result in (
                validation_results["quality_results"].get("checks", {}).items()
            ):
        ...
        if validation_results.get("test_results"):
            ...
            for test_type, result in (
                validation_results["test_results"].get("tests", {}).items()
            ):
                status_icon = "✓" if result.get("success", False) else "✗"
