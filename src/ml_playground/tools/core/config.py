...
class ToolsConfig(BaseModel):
    ...
    display_command_prefix: str | None = Field(
        default=None,
        description="Optional prefix (like `uv run`) displayed when narrating commands",
    )
...
def _load_pyproject_toml(project_root: Path) -> dict[str, object]:
    ...
    try:
        with open(pyproject_path, "rb") as f:
            # Use explicit cast to object then dict[str, object] to satisfy basedpyright
...
def load_tools_config(project_root: Path | None = None) -> ToolsConfig:
    ...
    tool_section = cast(dict[str, object], pyproject_data.get("tool", {}))
    ml_playground_section = cast(
        dict[str, object], tool_section.get("ml_playground", {})
    )
    tools_section = cast(dict[str, object], ml_playground_section.get("tools", {}))
...
def get_tool_config(category: str, project_root: Path | None = None) -> ToolConfig:
    ...
    return cast(ToolConfig, getattr(config, category))
