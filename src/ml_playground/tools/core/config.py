...
class ToolConfig(BaseModel):
    ...
    enabled: bool = Field(
        default=True,
        description="Whether this tool category is enabled"
    )
    timeout: int = Field(
    ...
    def validate_timeout(cls, v: int) -> int:
        ...
        if v > 3600:  # 1 hour maximum
            raise ValueError("Timeout too large - choose based on expected operation duration")
        return v
...
class QualityToolsConfig(ToolConfig):
    ...
    mypy_config_section: str = Field(
        default="tool.mypy",
        description="TOML section for mypy configuration"
    )
    ...
    strict_mode: bool = Field(
        default=True,
        description="Enable strict mode for type checking (BasedPyright)"
    )
...
class CIToolsConfig(ToolConfig):
    ...
    badge_output_dir: Path = Field(
        ...
        description="Directory for generated badges"
    )
...
class ToolsConfig(BaseModel):
    ...
    testing: TestToolsConfig = Field(
        default_factory=TestToolsConfig,
        description="Configuration for testing tools"
    )
    ...
    ci: CIToolsConfig = Field(
        default_factory=CIToolsConfig,
        description="Configuration for CI tools"
    )
    ...
    learning_mode_default: bool = Field(
        default=False,
        description="Whether learning mode is enabled by default"
    )
    default_verbosity: int = Field(
        default=1,
        description="Default verbosity level for learning mode (0-2)"
    )
