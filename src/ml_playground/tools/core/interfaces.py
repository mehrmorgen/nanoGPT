...
class OperationId(BaseModel):
    ...
    def validate_category(cls, v: str, info: ValidationInfo) -> str:
        ...
        # Use object type for namespace to satisfy basedpyright strict mode
        namespace_obj: object = info.data.get("namespace")
        if not isinstance(namespace_obj, str):
            raise ValueError("OperationId missing namespace during validation")

        valid_categories = valid_categories_by_namespace.get(namespace_obj)

        if valid_categories is None or v not in valid_categories:
            raise ValueError(
                f"Invalid {namespace_obj} category: {v}. Must be one of {valid_categories}"
            )
        return v
