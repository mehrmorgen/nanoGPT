...
def load_config_with_error_handling(
    ...
    *,
    ...
    try:
        # Allow tests to inject sentinel deps without load_config to bypass real loading.
        if deps is not None and not hasattr(deps, "load_config"):
            state.config = state.config or ToolsConfig()
