...
# Ensure pathlib.Path exposes _flavour for test Path subclasses on platforms where
# the attribute may be absent (e.g., newer Python internals).
try:
    _ = Path(".")._flavour  # type: ignore[attr-defined,unused-ignore]
except AttributeError:
    try:
        from pathlib import _PosixFlavour  # type: ignore

        Path._flavour = _PosixFlavour()  # type: ignore[attr-defined]
    except Exception:
        Path._flavour = None  # type: ignore[attr-defined]
...
class GitIgnorePattern(Protocol):
    include: bool

    def match_file(self, path: str) -> object | None: ...
...
def create_or_update_link(
    ...
    *,
    ...
    os_path_relpath: Callable[[Path | str, Path | str], str] | None = None,
    ...
    if not is_windows:
        try:
            if os_path_relpath:
                desired_link_repr = os_path_relpath(target_path, link_path.parent)
            elif os_relpath:
    ...
    if is_windows:
        ...
        try:
            if os_path_relpath:
                rel = os_path_relpath(target_path, link_path.parent)
            elif os_relpath:
                rel = os_relpath(target_path, link_path.parent)
            else:
    ...
    *,
    ...
    for entry in src_dir.iterdir():
        ...
        create_or_update_link(
            ...
            os_path_relpath=os_path_relpath,
            os_name=os_name,
    ...
    try:
        ...
        if create_or_update_link_op:
            create_or_update_link_op(
                ...
                path_readlink=path_readlink,
                os_relpath=os_relpath,
                os_path_relpath=os_relpath,
                os_name=os_name,
            )
        elif ensure_aiignore_symlink_op:
            ensure_aiignore_symlink_op(primary_path, readme, dry_run, logs, runner)
        else:
            create_or_update_link(
                primary_path,
                readme,
