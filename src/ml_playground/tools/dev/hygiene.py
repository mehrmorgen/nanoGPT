...
def run_kill_port(
    ...
    root_path: Path,
    *,
    pids_by_port: Callable[[int], Iterable[int]] | None = None,
    ...
    _pids = pids_by_port or _pids_by_port
    _kill = kill_pid or _kill_pid
    try:
        pids = list(dict.fromkeys(_pids(port)))  # dedupe, preserve order
        if not pids:
            return ToolResult.create(
                success=True,
                exit_code=0,
...
def _pids_by_port(port: int) -> list[int]:
    ...
    pids: set[int] = set()
    try:
        for conn in cast(Iterable[object], psutil.net_connections(kind="inet")):
            try:
                laddr: object = getattr(conn, "laddr", None)
                if not laddr:
                    continue
                conn_port = getattr(laddr, "port", None)
                conn_pid = getattr(conn, "pid", None)
                if conn_port == port and conn_pid is not None:
                    pids.add(int(cast(int | float | str, conn_pid)))
        ...
        try:
            process_iter = cast(
                ...
                psutil.process_iter,  # type: ignore[arg-type]
            )
            for proc in process_iter(attrs=["pid"]):
                try:
                    net_conns = cast(
                        Iterable[object], getattr(proc, "net_connections")(kind="inet")
                    )
                    proc_pid = int(cast(int | float | str, getattr(proc, "pid")))
                except Exception:
                    continue
                for c in net_conns:
