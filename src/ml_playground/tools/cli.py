...
def learn_commands(
    ...
    detailed: Annotated[
        bool,
        typer.Option("--detailed", help="Show detailed command descriptions")
    ] = False,
) -> None:
    ...
    if category:
        ...
        if category not in command_catalog:
            typer.echo(f"Error: Unknown category '{category}'. Valid categories: {', '.join(command_catalog.keys())}", err=True)
            raise typer.Exit(1)
        ...
        typer.echo(f"Use [bold]uv run tools {category} --help[/bold] for detailed command options.")
        typer.echo(f"Use [bold]uv run tools learn explain {category}.<command>[/bold] for educational content.")
        ...
        for cat, info in command_catalog.items():
            ...
            if detailed:
                typer.echo(f"    Commands: {', '.join(list(info['commands'].keys())[:5])}{'...' if len(info['commands']) > 5 else ''}")
        typer.echo("")
        
        typer.echo("[bold]Quick Start:[/bold]")
        typer.echo("  uv run tools env setup              # Set up development environment")
        ...
        typer.echo("  uv run tools learn commands --category <name>    # Show commands for category")
        typer.echo("  uv run tools learn explain <category>.<command>  # Get detailed explanations")
        typer.echo("  uv run tools learn best-practices               # Show best practices guide")
        typer.echo("  uv run tools <category> <command> --learning-mode # See explanations while running")
        typer.echo("")
        
        typer.echo("Use [bold]--detailed[/bold] for more information or [bold]--category <name>[/bold] for specific categories.")
...
def learn_explain(
    ...
    if '.' not in command:
        typer.echo(f"Error: Command must be in format 'category.command' (e.g., 'quality.lint')", err=True)
        raise typer.Exit(1)
    
    try:
        category, cmd = command.split('.', 1)
    except ValueError:
        typer.echo(f"Error: Invalid command format '{command}'. Use 'category.command'", err=True)
        raise typer.Exit(1)
    
    # Validate category
    valid_categories = {'quality', 'test', 'env', 'ci', 'agentic'}
    if category not in valid_categories:
        typer.echo(f"Error: Unknown category '{category}'. Valid categories: {', '.join(valid_categories)}", err=True)
        raise typer.Exit(1)
    ...
    learning_info = engine.explain_command(
        command=cmd,
        context=f"Explaining {category}.{cmd} command",
        category=category
    )
    ...
    if not any([learning_info.explanations, learning_info.best_practices, learning_info.related_concepts]):
        typer.echo(f"No educational content available for '{command}'")
        typer.echo("This command may not exist or educational content may not be implemented yet.")
...
def _show_learning_paths_overview(verbosity_level) -> None:
    ...
    if verbosity_level.value == 0:  # Minimal
        typer.echo("Use `uv run tools learn best-practices --category <name>` for specific guidance")
        return
    ...
    typer.echo("Use `--category <name>` to see detailed paths for specific tool categories.")
