...
- [Continuous Integration Guidelines](../../.dev-guidelines/project-specific/CI.md) – Platform-agnostic CI policies.
- [Development Practices](../../.dev-guidelines/project-specific/DEVELOPMENT.md) – Quality gates, tooling, and workflow expectations.
...
- **Purpose**: Fast gate validating linting, formatting, typing, and pytest tiers via `uv run tools ci quality-gate`.
...
- **Key steps**:
  - `uv sync --frozen --group all` guarded by a cache-hit check.
  - `uv run tools ci quality-gate` executes pre-commit, linting, typing, and pytest suites.
...
- **Purpose**: Runs mutation testing (`uv run tools ci mutation run`) and captures reports for analysis.
...
- **Key steps**:
  - Conditional `uv run tools env sync --group all` when `.venv` cache misses.
  - `uv run tools ci mutation run` executes the mutation suite.
